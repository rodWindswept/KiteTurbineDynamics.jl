...
| **Backline** | 1 line, sky hook ↔ backline ground anchor. **Partially elasticated in the field** — takes up slack, stays lightly tensioned, tightens hard only at the dyneema length. **Structure:** an altitude limiter for the sky hook. It is NOT a load path for the TRPT support, because it carries none of the rotor thrust or torque. **Condition (RULED, Rod, 2026-09-24):** it must stay taut, within its elastic range, at the design point, for clean operation of the kite turbine. Taut IS a load path requirement. |
