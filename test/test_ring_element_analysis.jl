...
@testset "ring_element_analysis" begin
    ...
    @testset "Test 5: self-equilibration warning fires for unbalanced loads" begin
        ...
        @test_warn "load imbalance" KiteTurbineDynamics.assemble_ring_frame(
            R, n, α, tp, F_local
        )
    ...
    @testset "Test 7: moment and torque equilibration under dynamic conditions" begin
        ...
        Mz = sum(
            xs[j] * F_trans_relieved[2, j] - ys[j] * F_trans_relieved[1, j] for j in 1:n
        )
    ...
    @testset "Test 9: out-of-plane moment relaxation filter" begin
        ...
        for j in 1:n
            ...
            K_locals[j] = KiteTurbineDynamics.beam_stiffness_local(
                tp.E, tp.G, tp.A, tp.I_bend, tp.J, L_beam
            )
            T_mats[j] = KiteTurbineDynamics.beam_transform(pa, pb, ring_normal)
        end

        # Case 1: oop_relaxation = 1.0 (raw unrelaxed moments)
        beams_raw = KiteTurbineDynamics.extract_beam_forces(
            d, R, n, α, tp, K_locals, T_mats, 1.0
        )

        # Case 2: oop_relaxation = 0.05 (compliant Dyneema relaxation factor)
        beams_relaxed = KiteTurbineDynamics.extract_beam_forces(
            d, R, n, α, tp, K_locals, T_mats, 0.05
        )
