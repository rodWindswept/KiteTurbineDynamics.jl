...
@testset "ring_spacing_v4 — constant L/r" begin
    ...
    Lr_vals = [(zs[i + 1] - zs[i]) / ((rs[i] + rs[i + 1]) / 2.0) for i in 1:n_segs]
...
@testset "TRPTDesignV4 — torsional FoS uniformity across rings" begin
    ...
    r = evaluate_design(
        d; r_rotor=p.rotor_radius, elev_angle=p.elevation_angle, omega_rotor=0.0
    )
