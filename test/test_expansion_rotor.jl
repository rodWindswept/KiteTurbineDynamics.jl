@testset "expansion rotor — analytical mechanics" begin
    ...
    @testset "zero-wind spreading via ω²" begin
        er = ExpansionRotorParams(3, 0.5, 0.1, 0.05, 1.0, 0.02, 0.05,
                                   15.0, 0.5, 3, 1.0)
        rho     = 1.225
        v_wind  = 0.0
    ...
    @testset "steeper bridle → more radial force" begin
        er_flat = ExpansionRotorParams(3, 0.5, 0.1, 0.05, 1.0, 0.02, 0.05,
                                        5.0, 0.5, 3, 1.0)
        er_steep = ExpansionRotorParams(3, 0.5, 0.1, 0.05, 1.0, 0.02, 0.05,
                                         40.0, 0.5, 3, 1.0)

        Fr_flat, Fa_flat, _, _, _ = expansion_rotor_forces(er_flat, 1.225, 10.0, 30.0, 20.0, 1.0, 500.0, 5)
        Fr_steep, Fa_steep, _, _, _ = expansion_rotor_forces(er_steep, 1.225, 10.0, 30.0, 20.0, 1.0, 500.0, 5)
