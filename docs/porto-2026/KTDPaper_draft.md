...
The rotary TRPT approach traces back to early multi-kite architecture concepts (Read, 2013) and a tested Daisy Kite model displayed at the 2015 Airborne Wind Energy Conference in Freiburg. The first practical demonstration followed in 2018, when a rigid-rotor Daisy Kite produced over 1 kW in flight testing — establishing that tensile torque transmission through a rotating tether network was achievable. Much of this early development was documented on web forums, establishing public-domain prior art. Tulloch (2021, 2023) subsequently formalised the steady-state TRPT model through the single-section Pyramid configuration, establishing the moment-to-tension ratio (MTR ≈ 0.05) that characterises the torque-tension coupling in a single TRPT section. Wacker (2022) extended this to a steady-state optimisation framework for the Daisy Kite design, demonstrating that multi-section TRPT geometries could be numerically optimised for power density. However, Wacker's model treated ring radii and segment lengths as fixed design parameters rather than free variables in a DE loop, and — critically — did not include dynamic verification. The rotor aerodynamics were handled by NREL's AeroDyn (BEM with Pitt-Peters yaw correction), while TRPT drag was computed through a segmented tether and frame drag model adapted from Tulloch.
...
Five parameters are at their search bounds — Do_top, t_over_D, r_bottom, and λ_bottom at minima; target_Lr and bank_bottom at maxima — indicating the true global optimum lies outside the current tight envelope. Wider bounds on structural minima would likely produce designs below 49 kg.
...
Wacker (2022) optimised the Daisy Kite design in steady state using NREL AeroDyn for rotor aerodynamics and a segmented TRPT drag model. His base case (13 hexagon frames, 3 rigid blades, TRPT version 4) produced power densities that — while not competitive with conventional wind — demonstrated the feasibility of systematic RAWES optimisation. KTD.jl extends Wacker's approach in three ways: (1) ring geometry is a free DE variable rather than a fixed design, (2) the optimisation includes discrete variables (n_lines, rotor mask), and (3) dynamic verification is integrated post-optimisation through the ODE verifier. Wacker's explicit exclusion of dynamic effects — "the lifter kite dynamics, as well as take-off and landing strategies will therefore be of minor importance" (Wacker 2022 §1.2) — identifies precisely the gap that the 4.2× discrepancy now quantifies.
...
The current study has several limitations. **Static solver scope.** The equilibrium solver captures rotor aerodynamics and TRPT geometry but not inter-ring torsional dynamics — the very physics responsible for the 4.2× gap. Closing this gap will require integrating the ODE verifier into the DE objective function, at significant computational cost (~100× per evaluation). **Bound-limited optimum.** Five parameters are at search bounds, indicating the true optimum lies outside the current envelope. Thinner beams (Do_top < 0.06 m), smaller ground rings (r_bottom < 2.0 m), and longer segments (target_Lr > 3.0 m) would reduce mass further but require manufacturing validation. **Dynamic verification scope.** The ODE verifier was run post-campaign on the single winner design. Full dynamic verification of the Pareto front would identify whether alternative designs close the static-dynamic gap. **K1 graph coverage.** The knowledge graph covers 540 papers — comprehensive for AWE but not exhaustive. Web validation (Phase 3b) mitigated this for high-risk claims, but niche prior art may remain unidentified.
...
8. Pfister, J.-L. & Blondel, F. (2020). BEM vs. free-vortex wake methods for thrust and power coefficients at inclined rotor discs. In *Proceedings of the Torque 2020 conference*.
9. Moore, M.D. (2014). *Centrifugally stiffened rotor: Eternal flight as the solution for 'X'*. NASA NIAC Phase I Final Report.
10. NASA (1995). *Closed-form static analysis with inertia relief and displacement-dependent loads*. NASA TM-1995-13233.
11. Tveide, T. *TetherDragODESolver*. GitHub repository. Validates ¼ tether drag coefficient for AWE.
12. Read, R. (2013). Multi-kite architecture concepts. Presentation, Berlin.
13. Read, R. (2015). Daisy Kite: a tested rotary AWE model. Airborne Wind Energy Conference, Freiburg.
14. Read, R. (various). Daisy Kite design and experimental data. Windswept & Interesting Ltd.
