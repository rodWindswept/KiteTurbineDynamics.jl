...
The V10 Tight campaign (12 islands $\times$ 1,500 iterations, tight bounds, $k_{\mathrm{mppt}} \propto \lambda^2$, ring-mapping fix) found a fundamentally different design basin. The winner—Island~1, 49.20~kg—employs 4 rotors distributed across the 10 intermediate rings (positions 1, 4, 7, and 10 counting from the bottom), with the hub rotor correctly occupying the top intermediate ring (Figure~\ref{fig:pca}).
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../awes-forum-diagrams/v10-tight-landscape.png}
\caption{PCA design space of the V10 Tight campaign. Two dominant axes capture 49.3\% of total variance. PC1 (28.9\%) captures structural scale ($r_{\mathrm{hub}}$, $D_{o,\mathrm{top}}$, $n_{\mathrm{lines}}$); PC2 (20.4\%) captures configuration choice ($\lambda$ gradient, bank gradient, rotor mask selection). The winner (marked) sits in a compact low-mass basin at $\mathrm{PC1} \approx -0.5$, $\mathrm{PC2} \approx -0.3$. White iso-mass contours at 50 and 65~kg show the steep mass gradient around the optimum, consistent with a constraint intersection rather than a broad valley.}
\label{fig:pca}
\end{figure}

Principal component analysis of the V10 Tight design space (Figure~\ref{fig:pca}) reveals two dominant axes capturing 49.3\% of total variance—significantly more than V10v1 (32.7\%), reflecting the concentrated search within tight bounds. PC1 (28.9\% variance) captures structural scale: $r_{\mathrm{hub}}$, $D_{o,\mathrm{top}}$, and $n_{\mathrm{lines}}$ dominate. PC2 (20.4\% variance) captures configuration choice: $\lambda$ gradient, bank gradient, and rotor mask selection. The winner sits in a compact low-mass basin at $\mathrm{PC1} \approx -0.5$, $\mathrm{PC2} \approx -0.3$. White iso-mass contours at 50 and 65~kg show the steep mass gradient around the optimum, consistent with a constraint intersection rather than a broad valley.
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../awes-forum-diagrams/v10-tight-nondim.png}
\caption{Non-dimensional physics atlas. PCA landscape coloured by nine Pi groups.}
\label{fig:nondim}
\end{figure}

The island convergence paths (Figure~\ref{fig:pca}) confirm that the multi-rotor basin is the dominant attractor: 58 of 60 DE islands independently converge to the same 49--75~kg region despite random initialisation, with Island~1 securing the global optimum at 49.20~kg.
