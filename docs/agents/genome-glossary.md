...
| x4 | Do_scale_exp | 0.0 | 1.0 | — | `design_from_vector_v4` | Tube diameter taper rate from hub toward ground. 0 = uniform Do along shaft; 1 = ground-ring Do → 0. Ground ring is supported by ground station — its mass is not airborne. | DE should taper aggressively (ground carries the mass). Gaming: zero taper wastes mass on intermediate rings that ARE airborne. |
