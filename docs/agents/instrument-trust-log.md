...
|------|-------|---------|-----------|------------|
| 2026-09-12 | **Bearing transverse damper lost in the canonical-loop refactor — the canonical loop has NO bearing damper** | The lift bearing starts 1.7 mm off the shaft axis and is 0.48 m off-axis with 9.8 m/s transverse speed within 0.5 s of the canonical loop; it settles into a slow undamped ~0.3–0.5 m wobble that never decays (`scratch/bearing_motion_probe.jl`). Rod remembers the bearing "whipping about unreasonably" and needing damping | `e83f6ca` (2026-05-18) added `bearing_tr_damp = 0.99994` deliberately — "Without transverse dissipation the bearing can precess indefinitely under asymmetric bridle loading" — but added it to `simulate()` in `initialization.jl`. `run_canonical_sim!` was born later in `12bc91b` and **the damper was never ported**: `git log -S "_bdretain" -- src/simulation.jl` is empty. So the canonical loop (used by the evaluator + campaign) damps rope nodes only, while the near-obsolete `simulate()` (4 scripts + 1 test) has the bearing damper | **OPEN.** Decision needed, not just a port: the 0.99994 figure is an empirical stand-in for drag on the fitting hardware, Dyneema viscoelasticity and rotor gyroscopic stabilisation, and Rod was never happy with it as unphysical. Options: port it, derive a physical value from the dissipations it stands for, or accept the wobble and gate it. See also the wobble-policy note in handover 2026-09-11 §6.2 |
| 2026-09-12 | **No test ever asserted lift-chain TENSION — and the one that came closest runs on the wrong machine** | The only live assertion of bridle tension is `test_bearing_alignment.jl:237` (`lowest_bridle.tension > 100.0`). It passes (216/216) on `params_10kw` with 1031–3492 N, but that is the **legacy 10 kW DRR machine**, not the v13 seed (`params_daisy`, 5 kW/18.8 m/6 lines). It also only reaches those numbers by dropping the hub 5 cm AFTER settling — it never asks whether the chain is taut *at the settle*. `git log -S "bridle_tension" -- test/` and `-S "cyan_tension"` return nothing, ever: the invariant `T > 0` for the lift chain has never existed as a test | Three compounding: (1) the invariant was never written; (2) 20 of ~50 live tests still use `params_10kw` and only 3 files carry the `ERA PIN — NOT a v13 health signal` header, so 17 assert physics on a machine the pipeline never builds **without warning**; (3) the parity test that would have flagged settle drift was deleted | Guard landed: `test/test_settle_validity.jl` (V1/V2/V3/V6, 2 pass + 5 `@test_broken`), wired into `runtests.jl` — `574dfaf`. Baseline: `scratch/settle_validity_baseline.jl`. **Still open:** era-pin the remaining `params_10kw` files, or migrate them to the daisy chain |
| 2026-09-12 | **Deleted parity test blamed for a regression it could not have found — and would not have** | `test/verify_initialization_consistency.jl` (deleted `cce274e`, 2026-07-14 "Week 1 cleanup") rebuilt the dashboard's hand-rolled settle and required `settle_to_operational_state` to match to 1e-15. Re-run against current code it FAILS with max diff **12.85** | The premise is **obsolete, not a regression**: the differing entries are rope nodes 224–240 and the sky anchor, exactly the nodes the settle's operational loop moves, and the dashboard **no longer contains a hand-rolled settle at all** — `interactive_dashboard.jl:537` calls `settle_to_operational_state` itself. So the test compared the library against a reconstruction of a method that no longer exists anywhere, and "the dashboard works" can never be evidence that the settle is valid, because they are one code path | Diagnostic kept: `scratch/parity_init_check.jl` (prints the block-wise diff; useful for localising settle drift). **Do not restore the test as a CI signal.** The real lesson: dashboard/library parity is not a validity check |
| 2026-09-12 | **Lift chain disconnected — bridles slack, lift never reaches the rotor** | On the 5 kW/18.8 m seed the 6 bridles (lift bearing → main rotor) read **0.0000 N** at the settle and at **all 25 samples** of a 6 s run; bearing–main-rotor distance oscillates 3.99–5.96 m against a 6.462198 m rest length. The lift is applied and never arrives. Sweeping lift-line elevation 40–90° does not restore it | Bridle rest length was derived from `bearing_offset = 6.0`, a **placeholder carried over from other tested systems** (Rod 2026-09-12) — the bearing's axial design point was never chosen. The correct geometry is ~3.99 m axial / ~4.66 m 3D (apex ~31° from axis, ~59° at the ring plane). Also: before `ec5e805` the preload restore replaced RING positions only and left the bearing/sky where the equilibrium settle put them; it now recomputes rings from `sd_r` while the lift chain re-equilibrates underneath | **OPEN.** Root cause located, fix not yet landed. See `docs/agents/physics-topology.md` §2–§3 and `scratch/preload_bridle_engagement.jl`, `scratch/preload_chain_search.jl`. Same failure mode as `DECISIONS.md:1806-1822` and `:2108-2120` |
| 2026-09-12 | **Hub axial budget counted only the main rotor's thrust — wrong by 6.6×** | A force budget using the main rotor alone failed to close by ~940 N and produced a spurious "no feasible preload exists" conclusion (and a consequently wrong ring-sizing recommendation) | The seed carries **3 rotors**. Main rotor +309.4 N axial thrust, but the expansion rotors on rings 8 and 7 add **+914.5 N and +820.5 N** — **2044.3 N total**, of which the main rotor is only ~15 %. Any hub/preload budget must sum every rotor | No code change; diagnostic corrected. `scratch/preload_thrust_budget.jl`. **Lesson recorded in `physics-topology.md` §5 item 5** |
| 2026-09-12 | **Spec update sign was inverted — fixed point diverged** | Plan §2.4 and handover 2026-09-11 §4 both printed `T_top -= 0.7·m_hub·a_ax` with the rationale "hub accelerating up-shaft ⇒ rope pulling too hard ⇒ reduce". The probe's converging branch was `update_sign=+1` | The rope hangs **below** the hub and pulls it **down**-shaft, so a hub accelerating up-shaft means that pull is too SMALL and `T_top` must RISE. The printed line faithfully implemented the inverted rule. Measured `df/dT = −1.073`; corrected iteration converges in 8 steps to 1155.09 N with contraction 0.1949 (the committed probe had stopped at 4 hard-coded iterations, 1158.97 N, residual −4.47 N, **not converged**) | Docs corrected `dc8daff`. **Lesson: verify a printed sign against measurement, and never treat a truncated iteration as converged** |
| 2026-09-12 | **`capture_extended().segment_twist_deg` is WRAPPED — hides twist saturation** | First-segment twist reported as "51.94°" (the number recorded as verification of the settle fix) while the stored α gives exactly **90.00°**, the `asin` clamp ceiling | `sim_frame.jl:414` wraps Δα into (−180°, 180°] (`mod(α[i+1]−α[i]+π, 2π)−π`). A segment wound past 90° reads as a small angle, so a saturated/clamped twist looks plausible | Use the stored α block (`u[6N+1 : 6N+Nr]`) when twist is large. **Lesson recorded in `physics-topology.md` §7** |
| 2026-09-12 | **`asin(clamp(·,−1,1))` silently returns 90°** | The closed form `sin(Δα) = τ·chord/(n_lines·T_s·r_a·r_b)` cannot exceed 1; when the requested torque is unreachable at the available tension, the function returns **90° with no error** | The realisability constraint is a **per-segment tension floor** `T_s ≥ τ·chord/(n_lines·r_a·r_b)` (1227.6 N … 1274.5 N of top tension on the seed, binding segment 4). Violating it is truncated rather than raised — structurally the same defect class as the `τ_carry ≤ 0` no-op | **OPEN** — must raise. See `physics-topology.md` §6 |
...
| 2026-08-24 | **~~Settle-vs-ODE 94× twist mismatch~~ → RETRACTED 2026-08-25: it was `J·θ`, not a rope-torque gap** | 2-rotor seed settles then reverses to ω<0; a first pass reported the ODE rope torque as 2.36 N·m vs the bisection's 221 N·m ("94×") | The "2.36 N·m" was `torques[1]` read AFTER the generator reaction was subtracted (a net residual, ~1% convergence error) — the chord model and ODE rope chain actually agree to 5 figures. The real cause is `ring_forces.jl:314`: `torques[ri] -= J_rotor·alpha[ri]` used the twist ANGLE θ as angular acceleration — a spurious torsional spring (J·θ) that braked the machine to reversal. Only fires when expansion rotors exist (why 1-rotor works, 2-rotor dies) | Blade mass is now rotary INERTIA (added to ring `inertia_z` at build, `dω/dt=τ/(I_z+J)`); J·θ term removed. Also fixed: preload used 3D chord as axial gap (over-tensioned 18–22×), bisection negative-twist floor |
| 2026-08-24 | **Settle ω-scan used expansion-rotor OFFSETS as absolute radii** | 2-rotor seed settled at ω=11.36 (too low); expansion rotor's swept area under-counted 5.8× (2.76 vs 15.96 m²) and TSR 2.4× in the settle scan | `initialization.jl` ω-scan did `area = π(blade_tip²−blade_hub²)` with `blade_tip`/`blade_hub` = 0.7·span / −0.3·span OFFSETS, dropping the ring's `r_nominal`. ODE/Betz/gate already used absolute radii — only the settle (and ramp evaluator `π·tip²`) were wrong | `expansion_annulus_area(er, r_nom)` + ODE mean-radius TSR; settle ω 11.36→13.88. Fast suite 1954/1954. 2-rotor stall PERSISTS (expansion τ(ω) non-monotonic → runaway reversal) — separate open item |
| 2026-08-24 | **Corrected-geometry seed NaN-poisoned settle → "stall" (3 wrong-geometry bugs, one class)** | Seed smoke-tested `P=0, FoS=Inf, ω pinned at 60 rad/s`; traced to `settle_to_operational_state`'s ω-scan returning the 60 rad/s fallback because its parasitic-drag term was NaN (`P_aero−NaN>P_gen` never true) | Geometric taper (`ring_spacing_v4`) makes NON-UNIFORM sub-segs (seed's shortest = 0.3119 m vs the 0.5 m `dt=4e-5` was calibrated for). Three consumers assumed uniform spacing/fixed dt: (1) settles keyed dt off `n_lines≥8` not Lmin → rope positions NaN at settle step 41; (2) axial-preload restore used uniform `seg_len=tether_length/n_seg` → bottom ring placed 2.15× out → 115% strain → break; (3) scoring window `V11_DT=4e-5` destabilised the 0.31 m sub-seg from the settled state → 41% strain spike → break | `stable_dt_for_system` = `min(4e-5, 4e-5·√(Lmin/0.5)/1.5)` (Rod: 1.5× margin), threaded through both settles AND the window; preload restore uses per-segment rest length + stiffness. Seed now runs to P=5.39 kW / 0.00% lift err, but FoS_min=1.01 (genuinely weak, needs re-seed). Fast suite green |
| 2026-08-24 | **Winner re-gate ran at a stale k_mppt (5.39, not the campaign's 2.24)** | The 5 kW campaign re-run's winner was re-gated and reported 'PASSES, P_gen 6.39 kW' — but that verdict was measured at k=5.39, a DIFFERENT machine than the campaign ran (k=2.24) | `scripts/ode_gate_v13.jl` hardcoded `k_mp = KW == 5.0 ? 5.39 : p.k_mppt` (the superseded 20 s-window sweep value).  The runner + smoke were updated to k=2.24 (honest-window sweep) but the gate was missed — the same k-alignment fault class as the 2026-08-13 'evaluator tested at k_mppt=10.0' row.  Caught in pre-push review (Hermes) | gate k → 2.24; winner re-gated at the campaign operating point: PASSES, P_gen 5.18 kW flat (5.09→5.18 over 30 s, ω 13.22 rad/s settled) — `026c734` |
| 2026-08-22 | **Non-finite FoS passes the mass-min floor (FIXED same day)** | `mass_min_fitness` (objective_v12.jl) and `v12_fitness` test `FoS_min < fos_hard` without an isfinite guard — `FoS_min = Inf` (null structural measurement) makes `Inf < 2.5` false, so a machine transmitting ≥5 kW with unmeasured ring loads passes the FoS floor and scores its mass; `v11_fitness` likewise scored FoS=Inf as the BEST fitness (-P) | exploit-register row 1 ("Inf FoS → feasible") was fixed ONLY in `objective_feasibility` (5d02d45); the mass-min objective (2026-08-20), v12_fitness and v11_fitness never got the guard. The running 5 kW campaign used the unguarded evaluator — telemetry showed zero FoS=Inf ok-rows; winners are screened at the re-gate | `(!isfinite(FoS_min) || FoS_min < fos_hard) && return Inf` in mass_min_fitness + v12_fitness; `!isfinite(FoS_min) && return Inf` in v11_fitness; test/test_fos_guard.jl (10 assertions) wired into runtests.jl — fast suite green |
