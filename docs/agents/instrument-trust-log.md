...
|------|-------|---------|-----------|------------|
| 2026-08-22 | **Non-finite FoS passes the mass-min floor (PENDING FIX — campaign monitoring)** | `mass_min_fitness` (objective_v12.jl:86) and `v12_fitness` (:29) test `FoS_min < fos_hard` without an isfinite guard — `FoS_min = Inf` (null structural measurement) makes `Inf < 2.5` false, so a machine transmitting ≥5 kW with unmeasured ring loads passes the FoS floor and scores its mass | exploit-register row 1 ("Inf FoS → feasible") was fixed ONLY in `objective_feasibility` (5d02d45); the 2026-08-20 mass-min objective and v12_fitness never got the guard. The running 5 kW campaign uses mass_min_fitness — telemetry is monitored for `ok` rows with FoS=Inf (none so far: seed FoS 36.42) | **PENDING** — `(!isfinite(FoS_min) || FoS_min < fos_hard) && return Inf` + RED test; land immediately after the campaign (docs/plans/2026-08-22-physics-convention-fixes.md §Sequencing item 0) |
