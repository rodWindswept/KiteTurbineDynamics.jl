...
|------|-------|---------|-----------|------------|
| 2026-08-24 | **Settle-vs-ODE twist-torque mismatch (~94×)** | 2-rotor seed settles then decelerates to ω<0 despite static net torque >0; ground ring receives 2.36 N·m from the rope where the bisection's τ_fn_a says 221 N·m | Bisection's simplified chord-torque (`T=EA·(chord−L)/L`, straight chord) does NOT match the ODE's discretised sub-segment rope torque — ~94× over-estimate. Settle winds the right twist per its model, but the ODE transmits ~100× less, so k·ω² outruns it → reversal. Candidate: sub-seg ring-end geometry, tension rectifier, or C1 torque clamp | NOT YET FIXED. Drag fix (n_lines=1 + curvature-0.5 removed) and bisection aero-injection (`τ_target=−τ_b−τ_exp`) landed — both correct, fast suite green — but do not clear the stall |
