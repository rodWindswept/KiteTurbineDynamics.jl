# Draft reply to Hong & Amjad — interim answers + honest findings

Subject: Re: your questions — answers, plus an honest finding on the geometry
...
Two model caveats we've also identified in the audit, so you have them up front: (1) the expansion-rotor blades currently contribute aerodynamic force but no rotational inertia or weight in the ODE (mass model calibrated in an earlier era) — so absolute FoS values and the spin-up duration are optimistic; comparisons between configurations remain valid since both share the simplification; (2) blade mass scaling has no per-blade-count term. Both are queued for the model-fix list and will be stated on any charts we send.
...
Looking forward to the QBlade + TRPT co-design discussion — this episode is actually a good argument for it.

Best,
Rod

---
*Note for Rod (strip before sending): numbers in para 5's last section deliberately avoid re-quoting the old kW figures as facts. If Hermes desktop finishes the re-run before you send, swap in the new numbers and cut the hedging sentence.*
