...
## Phase 2b — Blade-scaling traps: verified facts & required guards (2026-07-17, Hermes desktop)

Four traps were raised (Cowork session). Verdicts from reading the code on this machine:

1. **λ-reference shift — CONFIRMED.** `blade_scale` multiplies the *decoded* per-rotor blade dims (`builders_util.jl:95–97`), which embed the campaign gradient (λ_top≈0.52 → λ_bottom≈0.10). "blade_scale=0.85" on the 12-gon is 85% of *small* blades; on the legacy triangle it was 85% of *big* blades. Note `le = blade_scale` ALSO scales the hub BEM disk radius (5.0·le, line 108), main-blade mass (·le², line 113) and built-in km (·le², line 115) — it is not an expansion-blade-only knob. **No cross-config table at equal "λ" without the fingerprint (below).**
2. **n_blades = n_lines coupling — CONFIRMED** (`builders_util.jl:94`, Gate 1c). A forced n_lines=3 build gets 3 blades/rotor (4× less area than the 12-gon). For the *faithful phantom rebuild* this is self-consistent — the scrambled decode also produced 3 blades/rotor.
3. **Expansion blade mass — CONFIRMED, WORSE THAN FEARED.** `expansion_blade_mass(tip,λ) = (0.3+0.1·tip)·λ³` has **no n_blades dependence** (docstring claims "all n_blades blades" — calibrated for the 3-blade era). And `er.mass` never enters the ODE at all: ring nodes carry only `p.m_ring` (`initialization.jl:89`); only the *hub* rotor's `n_blades·m_blade` is dynamic mass. `er.mass` is bookkeeping (optimizer objective `objective_v10.jl:340/380`, analysis totals). So in the sim the 12-gon's 36 expansion blades add 12× aero force and **zero inertia/weight** — same held for the phantom's 9 blades, so old-vs-new comparisons are internally consistent, but absolute mass/FoS claims must say so.
4. **Double k-scaling — NO double application, but semantics matter.** The ODE and settle read **only `sys.k_mppt_ref[]`** (`ring_forces.jl:50–71`, `initialization.jl:741/750`). Built `km = k_base·λ²` merely seeds the ref at init; every sweep overwrites it with raw k (`control_map_hunt.jl:89/150`). So sweep "k=2" is an absolute N·m·s²/rad², identical across blade scales — λ² is applied zero times once overridden. **Rule: set `sys.k_mppt_ref[]` BEFORE any settle.**
   - **NEW BUG FOUND: `headless_verify.jl` k-scan is dead.** It builds `ctrl_test` with `k_mppt_test` (line 171) but never writes `sys.k_mppt_ref[]`, so every scan iteration's dynamics run at the seeded k≈615 (`params_v5_50kw`); only the *reported* P=k_test·ω³ varies. Fix: `sys.k_mppt_ref[] = k_mppt_test` inside the loop (+ restore after), with a regression test. Any future campaign gating through headless_verify mis-ranks k until this is fixed.

**Regression-gate adjustment (provenance):** `dynamic_verification.txt` (12.1 kW, ω=55.6 rpm; FoS 0.43 per `docs/plans/2026-06-28-control-first-design.md`) is dated **Jul 3** — it predates the 70/30 blade-offset errata (2026-07-05) and the settle/k_mppt-mismatch fix (2026-07-04, "invalidated all pre-fix dynamic results", technical-report §0/§2.7). Exact three-number reproduction with current code is therefore **not expected** and would be suspicious. Revised gates:

- **Gate A (12-gon, soft):** k=62 settle at 11 m/s uniform wind, all 4 rotors (no drop — matches `_build_verify_system`), no spokes, blade_scale=1.0. PASS = same verdict and magnitude: P ≈ 10–15 kW, ω ≈ 50–60 rpm, FoS < 1 (non-viable). Deviations attributed to the two post-Jul-3 physics fixes; record deltas.
- **Gate B (phantom triangle3, hard):** faithful rebuild of the scrambled decode (3 lines, 22 rings, untapered ~2.99 m, λ 1.0→0.88, bank 25°/4°, 3 blades/rotor) via a deliberate hand-pack of the JSON-order vector, documented as such. PASS = reproduces legacy kickstart 117 kW @ 11 m/s, k=2, blade 0.85 (FoS 4.5) within single-digit % — legacy CSVs were generated *after* both fixes, so this must match. Converts the Strathclyde numbers from "bug artifact" to "verified simulation of a now-deliberately-specified design".
- **Fingerprint (mandatory before any cross-config table):** builder emits at build time, and every sweep CSV carries (header comment or sidecar `.meta`): n_lines, n_rings, n_blades/rotor, per-rotor tip/hub radius, chord, per-rotor + total blade area, hub disk radius, per-rotor `er.mass` **and the note that er.mass is non-dynamic**, main-rotor n_blades·m_blade, decoded λ gradient, blade_scale, git hash. The builder prints what it built, every time.
