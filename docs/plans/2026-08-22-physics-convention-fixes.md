...
0. **DONE — non-finite-FoS guard (landed 2026-08-22, mid-campaign per
   Rod's sequencing choice).** `mass_min_fitness` and `v12_fitness` now
   test `(!isfinite(FoS_min) || FoS_min < cfg.fos_hard)`; `v11_fitness`
   rejects non-finite FoS (it previously scored FoS=Inf as the BEST
   fitness, -P).  test/test_fos_guard.jl (10 assertions) wired into
   runtests.jl; the old v11 test that ENCODED the exploit
   (`v11_fitness(100, Inf) == -100`) updated to the guarded behaviour.
   The running campaign's DE used the unguarded evaluator (zero FoS=Inf
   ok-rows in telemetry, monitored); winners are screened for the
   FoS=Inf signature at the re-gate.
