...
function test_shift()
    ...
    for ri in 1:Nr; u_start[6N + Nr + ri] = ω_rated; end
    u_start[6N + 1] = 0.0
    α_cum      = 0.0
    τ_target_a = τ_rated
    ...
    for s in 1:(Nr - 1)
        ...
        τ_fn_a = (Δα) -> begin
            ...
            for j in 1:p.n_lines
                ...
                pb_j    = attachment_point(ctr_b_test, nb.radius, α_cum + Δα,  j, p.n_lines, pp1, pp2)
    ...
    u_start[3*(sys.sky_anchor_id-1)+1 : 3*sys.sky_anchor_id] .-= cumulative_shift .* sd
    ...
    bgid = sys.bearing_id
    bacc = du[3*N+3*(bgid-1)+1 : 3*N+3*bgid]
    println("Bearing Acc directly after shift bisection: ", bacc)
    
    # Run operational settle 8000 steps with 0.15 damp
    ...
    for _ in 1:n_op
        fill!(du, 0.0)
        KiteTurbineDynamics.multibody_ode!(du, u_start, (sys, p, wf, ld), 0.0)
        @views u_start[3N+1:6N] .+= dt_op .* du[3N+1:6N]
        @views u_start[1:3N]    .+= dt_op .* u_start[3N+1:6N]
        @views u_start[3N+1:6N] .*= damp_op
        @views u_start[6N+Nr+1:6N+2Nr] .= ω_rated
        u_start[1:3]       .= 0.0
    ...
    fill!(du, 0.0)
    KiteTurbineDynamics.multibody_ode!(du, u_start, (sys, p, wf, ld), 0.0)
    bacc2 = du[3*N+3*(bgid-1)+1 : 3*N+3*bgid]
    println("Bearing Acc after operational settle: ", bacc2)
end
