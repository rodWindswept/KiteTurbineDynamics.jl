...
| File | Date | Topic |
|------|------|-------|
| `handover-2026-09-12-settle-rebuild-bow-and-static-solver.md` | Sep 12 | **CURRENT** — settle rebuild. Headline: **the shaft BOWS ~4.7° / ~1.5 m** (gravity's 124 N component perpendicular to the 30° axis), confirmed against the real machine, which reframes the settle as "find where a hanging column sits". The lift chain is **disconnected** (bridles 0.0000 N; rest 6.462198 m vs the 4.658 m the geometry wants); a resting state **does exist**; the **backline is the altitude limiter** taking the 143 N lift surplus; the **axial balance closes** (+66.6 → −0.32 N) and the lateral is the slow mode. `test/test_settle_validity.jl` guard landed (2 pass / 5 `@test_broken`). Static-solver harness fixed and verified (QR + per-DOF scaling; never form `J'J`) but the solve **does not converge** — diagnosis: the bow is a large geometric **line reorientation**, not a translation, and a global sideways ring motion is stiff (3.5e5 N/m). 28 commits ahead, unpushed |
| `handover-2026-09-12-session-state-and-preload-rootcause.md` | Sep 12 | **SUPERSEDED** by the above, except §2 (canonical Julia invocation). Session-state triage: all WIP committed; Claude's dead-knob root cause verified |
| `handover-2026-09-11-settle-ode-coherence.md` | Sep 11 | settle↔ODE coherence fixed (matched-place twist+axial solve; wind-up root cause corrected — axial preload, not frame softness). Acceptance 5/8: the corrected state does **not** stall, it exposes a **real FoS shortfall** (seed 1.31–2.31 vs a 2.5 floor) — do not re-baseline. **§1/§2 superseded 2026-09-12**; §3–§9 still valid |
