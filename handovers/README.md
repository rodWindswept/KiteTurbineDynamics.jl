...
| File | Date | Topic |
|------|------|-------|
| `handover-2026-09-12-session-state-and-preload-rootcause.md` | Sep 12 | **CURRENT** — session-state triage: the whole WIP is committed (`98c438c`, 10 ahead of origin, unpushed); `scripts/ktd-julia` / `scripts/ktd-format` are the canonical invocation; Claude's dead-knob root cause **verified** (`settle_to_equilibrium` leaves ω ≈ 0, so `τ_eq = 0`, and `_matched_place_twist` fails **silently** into the untwisted placement). Next: implement the kernel-derived preload fixed point with a hard failure, then re-derive loads / margin / window |
