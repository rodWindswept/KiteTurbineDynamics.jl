...
const LR_INIT_ZONES = [
    (0.4, 0.8), (0.7, 1.1), (1.0, 1.4), (1.3, 1.7), (1.6, 2.0),
]
...
function safe_objective_v5(x, beam, p, power_W, v_rated, elev_angle)
    ...
    r = evaluate_design(design;
        r_rotor=p.rotor_radius, elev_angle=elev_angle,
        v_peak=OPT_V_PEAK, fos_req=SAFE_EULER_FOS,
        v_rated=v_rated, P_rated=power_W)
...
function run_island(isl::Island, out_dir::String)
    ...
    write_hb = (elapsed_s, infeas_frac) -> begin
        ...
        row = DataFrame(
            timestamp=[string(now())], generation=[gen], evaluations=[evals],
            best_mass_kg=[best_mass], best_fos=[best_fos], best_tfos=[best_tfos],
            infeas_frac=[infeas_frac], elapsed_s=[elapsed_s],
            r_hub_m=[d.r_hub], r_bottom_m=[d.r_bottom], target_Lr=[d.target_Lr],
            n_lines=[d.n_lines],
        )
    ...
    t_start = time(); t_last_hb = t_start; done = false

    while gen < MAX_GENERATIONS && !done
        gen += 1
        infeas_count = 0; gen_best = Inf; gen_best_x = best_x

        for i in 1:POP_SIZE
            r1=r2=r3=i
            while r1==i||r2==i||r3==i||r1==r2||r1==r3||r2==r3
                r1=rand(rng,1:POP_SIZE); r2=rand(rng,1:POP_SIZE); r3=rand(rng,1:POP_SIZE)
            end
            ...
            for d in 1:D; (rand(rng)<DE_CR||d==jr) && (u[d]=v[d]); end
            f_u=obj(u); evals+=1; f_u>=1e6 && (infeas_count+=1)
            if f_u<fitness[i]; pop[i,:].=u; fitness[i]=f_u; end
            if fitness[i]<gen_best; gen_best=fitness[i]; gen_best_x=pop[i,:]; end
        end

        if gen_best < best_mass
            best_mass=gen_best; best_x=copy(gen_best_x)
            dg=design_from_vector_v4(best_x,beam,p)
            rg=evaluate_design(dg;r_rotor=p.rotor_radius,elev_angle=p.elevation_angle,
                               v_rated=v_rated,P_rated=power_W)
            best_fos=rg.min_fos; best_tfos=rg.min_torsional_fos; stall=0
        else
            stall+=1
        end

        elapsed=time()-t_start
        if time()-t_last_hb>=HEARTBEAT_PERIOD_S; write_hb(elapsed,infeas_count/POP_SIZE); t_last_hb=time(); end
        if gen%50==0
            @printf("[%s] island=%02d gen=%d evals=%d best=%.3fkg FOS=%.2f TFOS=%.2f stall=%d\n",
                    Dates.format(now(),"HH:MM:SS"),isl.idx,gen,evals,best_mass,best_fos,best_tfos,stall)
            flush(stdout)
        end
        if elapsed>=MAX_SEC_PER_ISLAND
            @printf("[%s] island=%02d budget reached\n",Dates.format(now(),"HH:MM:SS"),isl.idx)
            flush(stdout); done=true
        end
        if stall>=STALL_LIMIT_GEN && !done
            n_keep=max(1,Int(round(RESTART_KEEP_FRAC*POP_SIZE)))
            order=sortperm(fitness); elite=Set(order[1:n_keep])
            for i in 1:POP_SIZE
                i in elite && continue
                for d in 1:D; pop[i,d]=lo[d]+rand(rng)*(hi[d]-lo[d]); end
                fitness[i]=obj(pop[i,:]); evals+=1
            end; stall=0
            @printf("[%s] island=%02d stall restart\n",Dates.format(now(),"HH:MM:SS"),isl.idx)
            flush(stdout)
        end
    end
    ...
    r_best=evaluate_design(d_best;r_rotor=p.rotor_radius,elev_angle=p.elevation_angle,
                           v_rated=v_rated,P_rated=power_W)
    summary=DataFrame(
        island_idx=[isl.idx],cfg_name=[isl.cfg_name],beam_profile=[isl.bname],
        variant=[isl.variant],seed=[isl.seed],
        best_mass_kg=[best_mass],min_fos=[best_fos],min_torsional_fos=[best_tfos],
        feasible=[r_best.feasible && best_tfos>=SAFE_TORSION_FOS],
        r_hub_m=[d_best.r_hub],r_bottom_m=[d_best.r_bottom],target_Lr=[d_best.target_Lr],
        n_lines=[d_best.n_lines],tether_length_m=[d_best.tether_length],
        Do_top_m=[d_best.Do_top],t_over_D=[d_best.t_over_D],beam_aspect=[d_best.beam_aspect],
        Do_scale_exp=[d_best.Do_scale_exp],knuckle_kg=[d_best.knuckle_mass_kg],
        evaluations=[evals],elapsed_s=[elapsed],status=[r_best.constraint_msg],
    )
    CSV.write(best_path,summary)
    return (idx=isl.idx,mass_kg=best_mass,fos=best_fos,tfos=best_tfos,feasible=r_best.feasible&&best_tfos>=SAFE_TORSION_FOS)
end

function main()
    base_out=joinpath(@__DIR__,"results","trpt_opt_v5_safe"); mkpath(base_out)
    ...
    @printf("Torsion FOS≥%.1f  Euler FOS≥%.1f  r_bottom≥%.1fm\n",SAFE_TORSION_FOS,SAFE_EULER_FOS,SAFE_R_BOTTOM_MIN)
    println("CORRECTED: per-config power levels (10kW→10kW, 50kW→50kW)")
    println("Output: ",base_out); println("Started: ",now()); println("="^72); flush(stdout)

    results=NamedTuple[]
    for isl in islands
        @printf("\n▶ Island %02d/%d cfg=%-5s beam=%-10s var=%d seed=%d\n",
                isl.idx,N_ISLANDS,isl.cfg_name,isl.bname,isl.variant,isl.seed)
        flush(stdout)
        island_dir=joinpath(base_out,@sprintf("island_%02d",isl.idx))
        res=run_island(isl,island_dir); push!(results,res)
        @printf("✔ Island %02d mass=%.3fkg FOS=%.2f TFOS=%.2f\n",
                isl.idx,res.mass_kg,res.fos,res.tfos)
        flush(stdout)
    end
    CSV.write(joinpath(base_out,"campaign_summary.csv"),
        DataFrame(island=[r.idx for r in results], mass_kg=[r.mass_kg for r in results],
                  fos=[r.fos for r in results], tfos=[r.tfos for r in results],
