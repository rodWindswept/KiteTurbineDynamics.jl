...
const _V3_SIBLING = joinpath(
    dirname(ROOT), "relaxed-shamir-cd9a0f", "scripts", "results", "trpt_opt_v3"
)
# Use local if island data exists there, otherwise fall back to sibling worktree
const _V3_CHECK = joinpath(
    _V3_LOCAL, "10kw_circular_straight_taper_s1", "elite_archive.csv"
)
...
function pick_beam(s)
    return if s == "elliptical"
        PROFILE_ELLIPTICAL
    elseif s == "airfoil"
        PROFILE_AIRFOIL
    else
...
function render_v3(tag::String, out_path::String)
    ...
    for (i, r) in enumerate(radii)
        verts[i] = [[r*cos(2π*(k-1)/n_v), r*sin(2π*(k-1)/n_v), zs[i]] for k in 1:n_v]
    end
    ...
    ax3 = M.Axis3(
        ...
        titlecolor=:black,
        title=@sprintf(
            "v3 Winner — %s  %.2f kg  FOS %.2f  TorFOS %.2f  n=%d×%d  r_hub=%.2fm  taper=%.2f",
            uppercase(cfg),
            row.mass_kg,
            row.min_fos,
    ...
    for i in 1:length(verts)
        ...
        for k in 1:n_v
            a = ring[k];
            b = ring[mod1(k+1, n_v)]
            M.lines!(ax3, [a[1], b[1]], [a[2], b[2]], [a[3], b[3]]; color=col, linewidth=lw)
        end
    end
    ...
    for i in 1:length(verts)
        for p in verts[i]
            M.scatter!(
                ax3, [p[1]], [p[2]], [p[3]]; markersize=14, color=:red, strokewidth=0
            )
        end
    end
    ...
    return println("wrote $out_path   ($(row.mass_kg) kg, taper=$(row.taper))")
end

mkpath(OUT_DIR)
render_v3("10kw_circular_straight_taper_s1", joinpath(OUT_DIR, "fig_v3_winner_glmakie.png"))
