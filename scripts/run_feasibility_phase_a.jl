...
function save_row(gh, x, n_lines, n_active, f_v11, k_chosen, P_mean, FoS_min, ω_eq,
                  P_range, drifted, stationary, f_feas, tier, gen)
    row = Dict{Symbol,Any}(
...
function differential_evolution(pop, fit, gen)
    ...
    for i in 1:n
        ...
        _, _, P_mean, FoS_min, _, _, _, _, f_feas, tier, ok = evaluate_genome(trial)
        if !ok
            push!(new_pop, pop[i])
            push!(new_fit, fit[i])
            continue
        ...
        save_row(gh, trial, nl, na, f_feas > 10 ? Inf : f_feas, 0.0,
                 P_mean, FoS_min, 60.0, 0.0, false, false, f_feas, tier, gen)
...
function main()
    ...
    elseif FoS_fin >= 1.0
        ...
        println("RED — best FoS < 1.0 after $eval_count evals.")
        println("  Best: FoS=%.3f P=%.1f kW f_feas=%.3f", FoS_fin, P_fin, f_feas_fin)
        println("  Design family structurally underbuilt under corrected physics.")
    end
