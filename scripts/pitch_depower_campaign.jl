#!/usr/bin/env julia
# scripts/pitch_depower_campaign.jl
#
# High-Performance Pitch-Depower Parameter Sweep (V2 Campaign).
# Runs 512 headless simulations across 8 mechatronic and environmental axes,
# using a dynamic channel worker pool for 100% CPU thread efficiency,
# and enforces phase-aware safety disqualification metrics.
#
# Usage:
#   julia --project=. --threads=auto scripts/pitch_depower_campaign.jl
#
# Output files:
#   scripts/results/pitch_depower_campaign/campaign_metrics.csv  — summary table
#   scripts/results/pitch_depower_campaign/timeseries_<run_id>.csv — per-run timeseries
#
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Output directory ──────────────────────────────────────────────────────────
const RESULTS_DIR = joinpath(@__DIR__, "results", "pitch_depower_campaign")
mkpath(RESULTS_DIR)

# ── Sweep axes (512-run factorial V2 grid) ────────────────────────────────────
WIND_SPEEDS      = [11.0, 20.0]                     # m/s (Rated vs. Storm)
PAYOUT_DURATIONS = [2.0, 4.0, 8.0, 15.0]           # s (Fast burst to standard payout)
FIELD_IMU_VALS   = [false, true]                    # Active damping toggle
DAMPING_MODES    = [0.0, 2.0]                       # 0=MPPT, 2=LPF Speed Mode
ACTIVE_WINCH_VALS= [false, true]                    # Winch compliance
PAYOUT_BASES_M   = [15.0, 25.0]                     # Max payout quantity
MPPT_STALL_VALS  = [false, true]                    # Stall governor enabled
LIFTER_ELEVS_DEG = [70.0, 85.0]                     # Live sky-anchor elevation angles
...
function build_grid()
    ...
    for ws in WIND_SPEEDS
        for pdur in PAYOUT_DURATIONS
            for fimu in FIELD_IMU_VALS
                for dmode in DAMPING_MODES
                    for awinch in ACTIVE_WINCH_VALS
                        for payout in PAYOUT_BASES_M
                            for mstall in MPPT_STALL_VALS
                                for elev in LIFTER_ELEVS_DEG
                                    push!(grid, (
                                        wind_speed      = ws,
    ...
    # ── Longest Processing Time (LPT) Sorting ──
    # Sort by payout_duration descending (longest payout sweeps take the most time)
    # to minimize tail latency and balance thread workload.
    grid = sort(grid, by = x -> x.payout_duration, rev = true)
    return grid
end

# Smoke test grid: 3 runs spanning axes
function build_smoke_grid()
    [
        (wind_speed=11.0, payout_duration=2.0, field_imu=false, damping_mode=0.0, active_winch=false, payout_base_m=15.0, mppt_stall=false, lifter_elev_deg=70.0, duration_s=10.0),
        (wind_speed=20.0, payout_duration=4.0, field_imu=true,  damping_mode=2.0, active_winch=true,  payout_base_m=25.0, mppt_stall=true,  lifter_elev_deg=85.0, duration_s=10.0),
        (wind_speed=11.0, payout_duration=15.0, field_imu=true,  damping_mode=2.0, active_winch=false, payout_base_m=15.0, mppt_stall=false, lifter_elev_deg=85.0, duration_s=10.0),
    ]
end
...
function derive_metrics(res::DepowerResult, config::NamedTuple, run_id::Int)
    ...
    if n < 2
        return (run_id=run_id, config...,
                ...
                smoothness_raw=NaN, tension_raw=NaN, composite_score=-99999.0)
    end
...
function run_one(run_id::Int, config::NamedTuple; save_ts::Bool = true)
    try
        ...
        # Wire wind speed and lifter elevation dynamically
        ...
        res = run_pitch_depower!(copy(u_s), sys, p_base, wind_fn, n_steps, dt;
            ...
            depower_sequence = 3, # Ssweeps focused on Sequence 3 (Lift -> Stall)
            payout_duration  = config.payout_duration)
...
function main()
    ...
    @info "Pitch Depower Campaign V2" n_runs=n_runs threads=nthreads() smoke_test=smoke_test
    ...
    # Feeds jobs to threads dynamically to prevent starvation and thread idle times.
    ...
    @info @sprintf("Campaign V2 complete: %d runs in %.1f minutes", n_runs, elapsed_total / 60)
