...
function main()
    ...
    rows = [eval_baseline("10 kW", params_10kw()), eval_baseline("50 kW", params_50kw())]
    df = DataFrame(rows)
    ...
    for row in rows
        ...
        @printf(
            "  Hub ring radius   : %.3f m    (taper_ratio=%.3f)\n",
            row.r_hub_m,
            row.taper_ratio
        )
        @printf("  Number of rings   : %d\n", row.n_rings)
        @printf(
            "  Baseline Do_top   : %.2f mm  (t/D=%.3f, √R scaling)\n",
            row.Do_top_mm,
            row.t_over_D
        )
        @printf("  Peak 25 m/s thrust: %.0f N (CT=1.0)\n", row.T_peak_N)
        @printf(
            "  Baseline mass     : %.3f kg (beams %.3f + knuckles %.3f)\n",
            row.mass_total_kg,
            row.mass_beams_kg,
            row.mass_knuckles_kg
        )
