...
param_cols = ["Do_top_m", "t_over_D", "beam_aspect", "Do_scale_exp",
              ...
              hasproperty(feas, :blade_scale_top) ? "blade_scale_top" : "lambda_top",
              hasproperty(feas, :blade_scale_bottom) ? "blade_scale_bottom" : "lambda_bottom"]
...
open("/tmp/v10_tight_atlas.csv", "w") do f
    ...
    for i in 1:3:nrow(feas)
        ...
        bs_top = hasproperty(feas, :blade_scale_top) ? feas.blade_scale_top[i] : feas.lambda_top[i]
        write(f, "$(pc1[i]),$(pc2[i]),$(feas.mass_kg[i]),$(round(feas.n_lines[i])),$n_rotors,")
        write(f, "$(feas.r_hub_m[i]),$(feas.r_bottom_m[i]),$(feas.bank_top[i]),")
        write(f, "$bs_top,$(feas.t_over_D[i]),$(feas.target_Lr[i]),$(feas.Do_top_m[i])\n")
    end
end
