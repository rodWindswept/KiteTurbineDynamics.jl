...
# Pitch-Depower Parameter Sweep (V4 Campaign).
#
# Sweeps over optimized Wind Speeds, Tether Stiffness, Tether Damping, Payout Durations,
# and Winch compliance using the newly validated dynamic relative-vector lagged kite model.
# Enforces strictly stable 100 kHz time-step, analytically settled state initialization,
# and dynamic whiplash/structural diagnostics.
#
# Usage:
#   julia --project=. --threads=auto scripts/pitch_depower_campaign_v4.jl
#
...
# ── Output directory ──────────────────────────────────────────────────────────
const RESULTS_DIR = joinpath(@__DIR__, "results", "pitch_depower_campaign_v4")
mkpath(RESULTS_DIR)

# ── Sweep axes (128-run V4 Campaign grid) ──────────────────────────────────────
WIND_SPEEDS      = [6.0, 11.0]                      # m/s (Rated compliance boundary vs. storm wind)
PAYOUT_DURATIONS = [1.5, 5.0]                      # s (Fast compliance transient vs. standard)
ACTIVE_WINCH_VALS= [false, true]                    # Winch compliance toggle
DAMPING_MODES    = [0.0, 2.0]                       # 0=MPPT, 2=LPF Speed Damping Mode
...
# Build V4 dynamic sweep factorial grid
function build_grid()
    grid = NamedTuple[]
                        ...
                        for c in C_BACK_LINES
                            for i_pto in I_PTO_VALS
                                push!(grid, (
                                    ...
                                    c_back_line     = c,
                                    i_pto           = i_pto,
                                    field_imu       = true,  # Essential for IMU closed-loop safety
                                    mppt_stall      = false, # Stall governor is bypassed in sequence 3
                                    payout_base_m   = 15.0,  # Standard payout displacement base
                                    lifter_elev_deg = 75.0,  # Fixed optimal elevation angle
                                    duration_s      = 10.0,  # Compact dynamic window (settled analytically)
                                ))
                            end
                        end
                    end
...
function build_smoke_grid()
    [
        (wind_speed=6.0,  payout_duration=1.5, active_winch=false, damping_mode=0.0, EA_back_line=350000.0, c_back_line=250.0, i_pto=12.5, field_imu=true, mppt_stall=false, payout_base_m=15.0, lifter_elev_deg=75.0, duration_s=2.0),
        (wind_speed=11.0, payout_duration=5.0, active_winch=true,  damping_mode=2.0, EA_back_line=700000.0, c_back_line=500.0, i_pto=25.0, field_imu=true, mppt_stall=false, payout_base_m=15.0, lifter_elev_deg=75.0, duration_s=2.0),
    ]
end
...
function derive_metrics(res::DepowerResult, config::NamedTuple, run_id::Int)
    ...
    if n < 2
        return (run_id=run_id, config...,
                ...
                smoothness_raw=NaN, tension_raw=NaN, composite_score=-999.0)
    end
    ...
    # Slack events counting (Total and Late-payout t > 4.15s)
    slack_ev   = sum(n_sl .> 0)
    ...
    # Composite Score: Strict penalty boundary applied
    composite = is_disqualified ? -999.0 : -smoothness_raw + 0.01 * tension_raw
...
function run_one(run_id::Int, config::NamedTuple; save_ts::Bool = true)
    try
        ...
        # Dynamically override grid parameters
        ...
        # This bypasses the settling dynamic run, giving a 3x speedup!
        u_s = settle_to_operational_state(sys, u0, p_base, ω_rated;
                    lift_device = lift_dev, wind_fn = wind_fn)

        # ── Step B: Execute Dynamic 100 kHz Solver ──
        dt = 1.0e-5 # Strictly stable time-step (100 kHz)
...
function main()
    ...
    @info "Pitch Depower Campaign V4" n_runs=n_runs threads=nthreads() smoke_test=smoke_test
    ...
    @info @sprintf("Campaign V4 complete: %d runs in %.1f minutes", n_runs, elapsed_total / 60)
