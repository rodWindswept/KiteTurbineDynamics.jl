...
println(
    "Hub node global id: $hub_id, nominal elevation: $(round(u_base[3*(hub_id-1)+3], digits=2)) m",
)
...
for (name, dev) in devices
    ...
    ode_params = dev === nothing ? (sys, p10, wind_3d) : (sys, p10, wind_3d, dev)
    ...
    for step in 1:(n_settle + n_sim)
        ...
        if step > n_settle && mod(step - n_settle, record_every) == 0
            ...
            push!(ts_rows, (name, t_rec, hx, hy, hz, r_xy, elev_d, omega_hub, omega_gnd))
        end
    end
    ...
    push!(
        smry_rows,
        (name, mean(hz_vec), std(hz_vec), std(rxy_vec), mean(el_vec), std(el_vec)),
    )

    @printf "  hub_z: mean=%.2f m  std=%.3f m  elev: mean=%.1f deg  std=%.2f deg\n" mean(
        hz_vec
    ) std(hz_vec) mean(el_vec) std(el_vec)
end
