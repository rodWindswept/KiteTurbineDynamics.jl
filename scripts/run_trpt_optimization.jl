...
function parse_config(name::String)
    ...
    return lname == "50kw" ? ("50 kW", params_50kw()) : ("10 kW", params_10kw())
end
...
function heartbeat!(
    ...
    CSV.write(
        logger.path, row; append=logger.initialized, writeheader=(!logger.initialized)
    )
    return logger.initialized = true
end
...
function save_checkpoint(path::String, state::OptState)
    open(path, "w") do io
        return Serialization.serialize(io, state)
    end
end
...
function de_step!(
    ...
    for i in 1:N
        ...
        v =
            state.population[r1, :] .+
            DE_F .* (state.population[r2, :] .- state.population[r3, :])
        # Boundary handling (reflect)
        for d in 1:D
            if v[d] < lo[d]
                ;
                v[d] = lo[d] + (lo[d] - v[d]) * 0.5;
            end
            if v[d] > hi[d]
                ;
                v[d] = hi[d] - (v[d] - hi[d]) * 0.5;
            end
...
function main()
    ...
    if state === nothing
        ...
        println(
            ">>> Resumed from checkpoint: gen=$(state.generation), ",
    ...
    obj_fn =
        x -> objective(
            x, profile, sys_params; rotor_radius=rotor_radius, elev_angle=elev_angle
        )
    ...
    for gen in gen_start:args["max-generations"]
        ...
        if gen % 50 == 0
            @printf(
                "[%s] gen=%d evals=%d best_mass=%.3f kg FOS=%.2f stall=%d\n",
                string(now()),
                gen,
                state.evaluations,
                state.best_mass,
    ...
    write_best_design_json(
        joinpath(out_dir, "best_design.json"), state, d_best, r_best, cfg_name
    )
    println("=" ^ 72)
...
"""
function write_best_design_json(
    path::String, state::OptState, design::TRPTDesign, r::EvalResult, cfg_name::String
)
    open(path, "w") do io
        println(io, "{")
