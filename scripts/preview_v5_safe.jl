...
function run(SAFE_TFOS, SAFE_EFOS, SAFE_RBOT, label)
    ...
    function eval_safe(x)
        ...
        return if (r.feasible && r.min_torsional_fos >= SAFE_TFOS)
            r.mass_total_kg
        else
            1e6 + r.mass_total_kg
        end
    ...
    for gen in 1:300
        ...
        bi = argmin(fitness)
        if fitness[bi] < best_mass
            ;
            best_mass = fitness[bi];
            best_x = copy(pop[bi, :]);
        end
    end
    ...
    @printf(
        "%s: %5.1f kg  r_b=%.2f  n=%d  EFOS=%.1f  TFOS=%.1f  %s\n",
        label,
        feas ? rb.mass_total_kg : Inf,
        db.r_bottom,
        db.n_lines,
        rb.min_fos,
        rb.min_torsional_fos,
        feas ? "✓" : "✗"
    )
end
