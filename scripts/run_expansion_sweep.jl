...
function main()
    ...
    total =
        length(N_EXPANSION_VALS) *
        length(BRIDLE_ANGLE_VALS) *
        length(BLADE_RADIUS_VALS) *
        length(BLADE_COUNT_VALS) *
        length(POWER_KW_VALS) *
        length(PLACEMENT_MODES)

    println("Expansion Rotor Parameter Sweep")
    println("===============================")
    println("Configs: $total")
    println(
        "Dimensions: N_exp × bridle_angle × blade_radius × blade_count × power × placement"
    )
    println()
                ...
                for bridle_angle in BRIDLE_ANGLE_VALS
                    for n_exp in N_EXPANSION_VALS
                        for placement in PLACEMENT_MODES
                            ...
                            stack = if (cfg !== nothing)
                                build_expansion_stack(cfg)
                            else
                            ...
                            try
                                ...
                                sys, _ = build_kite_turbine_system(
                                    p; expansion_rotors=stack
                                )
                                ...
                                @warn "Failed config $count: n_exp=$n_exp bridle=$bridle_angle° r=$blade_radius n_blades=$blade_count P=$power_kw kW $placement" exception =
                                    e
                            end
    ...
    for pk in POWER_KW_VALS
        ...
        if nrow(subset) > 0
            ...
            println(
                "  $(pk) kW:  baseline φ=$(round(phi_baseline; digits=3))  best φ=$(round(phi_best; digits=3))  improvement=$(round(phi_improvement; digits=1))%",
            )
        end
    end
end  # function main
