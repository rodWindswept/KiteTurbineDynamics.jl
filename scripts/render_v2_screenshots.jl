...
function build_design_from_archive_row(
    row, beam_profile::BeamProfile, axial_profile::AxialProfile, tether_length::Float64
)
    return TRPTDesignV2(
        beam_profile,
        row.Do_top_m,
...
function render_design(
    design::TRPTDesignV2, sys::SystemParams, out_path::String, title::String
)
    ...
    n_v = design.n_lines
    r_ev = evaluate_design(design; r_rotor=sys.rotor_radius, elev_angle=sys.elevation_angle)
    ...
    for i in 1:length(verts)
        ...
        for k in 1:n_v
            ...
            b = ring[mod1(k+1, n_v)]
            M.lines!(
                ax3, [a[1], b[1]], [a[2], b[2]], [a[3], b[3]]; color=:cyan, linewidth=2.0
            )
        end
    end
    # Axial lines (tethers)
    ...
    ax_rz = M.Axis(
        fig[1, 3];
        title="r(z) — $(axial_profile_name(design.axial_profile))",
        titlecolor=:white,
        backgroundcolor=:black,
        xlabel="r (m)",
        ylabel="z (m)",
    ...
    ax_fos = M.Axis(
        fig[2, 3];
        title="per-ring FOS (dashed = required 1.8)",
        titlecolor=:white,
        backgroundcolor=:black,
        xlabel="ring index",
        ylabel="FOS",
...
function main()
    ...
    tags = filter(readdir(V2_DIR)) do d
        return isdir(joinpath(V2_DIR, d)) &&
    ...
    for tag in tags
        try
            ...
            if nrow(df) == 0
                println("skip $tag — empty archive");
                continue
            end
