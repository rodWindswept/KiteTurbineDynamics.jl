...
function main(; top_k::Int=5)
    ...
    println(
        "wrote $csv_path ($(nrow(out_rows)) rows, $n_pass/$n_total designs pass envelope)"
    )
    ...
    summary = Dict(
        ...
        "envelope_points" =>
            [Dict("label"=>l, "v_peak"=>v, "omega_frac"=>o) for (l, v, o) in ENVELOPE],
    )
    open(joinpath(OUT, "phase_e_envelope_summary.json"), "w") do io
        return JSON3.pretty(io, summary)
    end
    return println("wrote $(joinpath(OUT, "phase_e_envelope_summary.json"))")
end

main(; top_k=(s -> parse(Int, s))(get(ENV, "PHASE_E_TOPK", "5")))
