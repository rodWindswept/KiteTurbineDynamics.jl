...
function run_island(isl::Island, out_dir::String)
    ...
    obj = x -> objective_v5(x, beam, p;
                             power_W=50_000.0, v_rated=12.0,
    ...
    r0        = evaluate_design_v5(d0; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle)
    best_fos  = r0.min_fos
    ...
    while gen < MAX_GENERATIONS && !done
        ...
        if gen_best < best_mass
            ...
            rg = evaluate_design_v5(dg; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle)
            best_fos = rg.min_fos
            stall    = 0
        else
            stall += 1
    ...
    r_best = evaluate_design_v5(d_best; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle)
