...
#
# Output: scripts/results/trpt_opt_v5/
...
function run_island(isl::Island, out_dir::String)
    ...
    obj =
        x -> objective_v5(
            x, beam, p; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle
        )
    ...
    r0 = evaluate_design_v5(
        d0; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle
    )
    best_fos = r0.min_fos
    ...
    while gen < MAX_GENERATIONS && !done
        ...
        if gen_best < best_mass
            ...
            rg = evaluate_design_v5(
                dg; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle
            )
            best_fos = rg.min_fos
            stall = 0
        else
        ...
        if gen % 50 == 0
            @printf(
                "[%s] island=%02d gen=%d evals=%d best=%.3fkg FOS=%.2f stall=%d\n",
        ...
        if elapsed >= MAX_SEC_PER_ISLAND
            @printf(
                "[%s] island=%02d budget reached at gen %d\n",
                Dates.format(now(), "HH:MM:SS"),
                isl.idx,
                gen
            )
        ...
        if stall >= STALL_LIMIT_GEN && !done
            ...
            stall = 0
            @printf(
                "[%s] island=%02d stall restart gen=%d\n",
                Dates.format(now(), "HH:MM:SS"),
                isl.idx,
                gen
            )
    ...
    r_best = evaluate_design_v5(
        d_best; power_W=50_000.0, v_rated=12.0, elev_angle=p.elevation_angle
    )
...
# ── Main ──────────────────────────────────────────────────────────────────────
function main()
    base_out = joinpath(@__DIR__, "results", "trpt_opt_v5")
    mkpath(base_out)
    ...
    for isl in islands
        ...
        @printf(
            "✔ Island %02d done  mass=%.3f kg  FOS=%.2f  feasible=%s\n",
            isl.idx,
            res.mass_kg,
            res.fos,
            res.feasible
