...
function build_design_from_row(
    row, beam::BeamProfile, axial::AxialProfile, tether_length::Float64
)
    return TRPTDesignV2(
        beam,
        row.Do_top_m,
...
function render_one(tag::String, out_path::String)
    ...
    ax3 = M.Axis3(
        ...
        titlecolor=:black,
        title=@sprintf(
            "%s — %.2f kg, FOS %.2f, n_rings=%d, n_lines=%d, r_hub=%.2fm",
            tag,
            row.mass_kg,
            row.min_fos,
            design.n_rings,
    ...
    return println(
        "wrote $out_path   ($(basename(tag)), $(round(row.mass_kg, digits=2)) kg)"
    )
end

function main()
    targets = ["10kw_circular_straight_taper_s1", "50kw_circular_straight_taper_s1"]
    for tag in targets
