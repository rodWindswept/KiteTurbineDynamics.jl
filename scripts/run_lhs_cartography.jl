...
function parse_config(n)
    return lowercase(n) == "50kw" ? ("50 kW", params_50kw()) : ("10 kW", params_10kw())
end
function parse_beam(n)
    return (
        if lowercase(n) == "elliptical"
            PROFILE_ELLIPTICAL
        elseif lowercase(n) == "airfoil"
            PROFILE_AIRFOIL
        else
            PROFILE_CIRCULAR
        end
...
function main()
    ...
    N = args["samples"]
    println(
        "LHS cartography: N=$N, config=$cfg_name, beam=$beam, axial=$(ax_locked === nothing ? "free" : ax_locked)",
    )
    ...
    println(
        "wrote $(args["output"]) — feasible=$feasible_count/$N ($(round(100*feasible_count/N;digits=1))%)",
    )
    return println("feasible min_mass = $(round(feasible_min_mass;digits=3)) kg")
end
