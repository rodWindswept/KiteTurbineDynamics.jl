...
function total_twist_deg(u)
    a = @view u[(6N + 1):(6N + Nr)]
    return rad2deg(sum(i -> mod(a[i + 1]-a[i]+pi, 2pi)-pi, 1:(Nr - 1)))
end
...
function run_scenario!(label, v_wind, lift_dev)
    ...
    for chunk in 0:N_CHUNKS
        ...
        if hub_z(u) < 3.0
            ...
            println(
                "  *** COLLAPSE at t=$(round(t,digits=2))s: hub_z=$(round(hub_z(u),digits=2))m ***",
            )
            collapsed = true
            break
        end
    ...
    if !collapsed
        println(
            "  t=$(round(t,digits=1))s: hub=($(round(hub_x(u),digits=2)), $(round(hub_z(u),digits=2)))m" *
