...
for (do_s, t_s) in [(1.0, 1.0), (1.5, 2.0)]
    sys, u0, p, _ = build_v10_tight(blade_scale=0.95, tether_diameter=0.004, r_bottom_scale=1.30, do_scale=do_s, t_scale=t_s)
    println("=== do=$do_s t_overD=$t_s  sys.Do_top=$(sys.ring_Do_top[])  sys.toverD=$(sys.ring_toverD[]) ===")
    ...
    for k in [2.0, 4.0, 8.0]
        sys.k_mppt_ref[] = k
        u = settle_to_operational_state(sys, copy(u0), p, 4.5; wind_fn=wf)
        for seg in 1:2
