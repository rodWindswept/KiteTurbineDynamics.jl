# Deep-Dive Pitch Depower Dynamics & Diagnostics Report
...
By analyzing the shaft speed differential ($\Delta\omega = \omega_{\text{hub}} - \omega_{\text{gnd}}$) in the steady depowered state ($t \in [17, 20]$s), we performed a Fast Fourier Transform to extract the dominant limit-cycle frequencies:
...
> In a perfect, tensioned TRPT shaft, torque is transmitted via the geometric tension of the tethers ($G J \propto T_{\text{line}}$). However, during Pitch Depower payout to **25m**, the backline length increases so much that the tethers go completely slack ($T < 5$ N) across the upper segments (Seg 10-17).
...
At $t = 18.5$s (fully depowered), the tether tension profile shows exactly where tension is lost:
