...
smry_all = DataFrame(;
    ...
    P_cv_pct=Float64[],
    omega_mean=Float64[],
    corr_time_s=Float64[],   # autocorr time of hub_z
)

psd_all = DataFrame(;
    v_wind=Float64[], device=String[], freq_hz=Float64[], psd_m2_per_hz=Float64[]
)
        ...
        run_canonical_sim!(
            ...
            callback=(u_curr, t_curr, step) -> begin
                if mod(step, REC_EVERY) == 0
                    ...
                    push!(
                        local_ts,
                        (t_curr, hx, hy, hz, r_xy, elev, ω_hub, ω_gnd, τ_gen, P_kw, v_inst),
                    )
                end
            end,
        )
...
for r in eachrow(smry_all)
    @printf "  %6.1f  %-14s  %13.2f  %10.2f  %7.1f  %8.2f\n" r.v_wind r.device (
        r.hub_z_std*1000
    ) r.P_mean_kw r.P_cv_pct r.corr_time_s
end
