...
function CircularTube(Do::Float64, t_over_D::Float64)
    return CFRPTube(CircularProfile(Do, t_over_D), DEFAULT_CFRP)
end
function EllipticalTube(Do::Float64, t_over_D::Float64, aspect::Float64)
    return CFRPTube(EllipticalProfile(Do, t_over_D, aspect), DEFAULT_CFRP)
end
function AirfoilTube(Do::Float64, t_over_D::Float64, aspect::Float64)
    return CFRPTube(AirfoilProfile(Do, t_over_D, aspect), DEFAULT_CFRP)
end
...
"""
function strut_properties(
    tube::CFRPTube{CircularProfile}, L::Float64, ends::AbstractEndCondition
)::StrutProperties
    Do = tube.profile.Do
...
function strut_properties(
    tube::CFRPTube{EllipticalProfile}, L::Float64, ends::AbstractEndCondition
)::StrutProperties
    Do = tube.profile.Do
    t = max(tube.profile.t_over_D * Do, OPT_T_MIN_WALL)
    ...
    perim =
        ...
        (
            1.0 +
            3.0 * ((a - b) / (a + b))^2 / (10.0 + sqrt(4.0 - 3.0 * ((a - b) / (a + b))^2))
        )
...
function strut_properties(
    tube::CFRPTube{AirfoilProfile}, L::Float64, ends::AbstractEndCondition
)::StrutProperties
...
function utilisation(
    tube::CFRPTube, props::StrutProperties, N::Float64, M_ip::Float64, M_oop::Float64
)::Float64
