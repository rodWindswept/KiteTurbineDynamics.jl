...
function search_bounds_v6(p::SystemParams, beam_profile::BeamProfile;
                          max_ground_radius::Float64 = OPT_MAX_GROUND_RADIUS)
    # Base v5 bounds (returns (lo, hi) vectors)
    base_lo, base_hi = search_bounds_v5(p, beam_profile; max_ground_radius=max_ground_radius)
...
function design_from_vector_v6(x::AbstractVector, beam_profile::BeamProfile,
                                p::SystemParams;
                                max_ground_radius::Float64 = OPT_MAX_GROUND_RADIUS)
    # Base v5 design (first 9 vars)
    design = design_from_vector_v5(x[1:9], beam_profile, p;
...
"""
function estimate_effective_radii(design::TRPTDesignV4,
                                   stack::Vector{ExpansionRotorParams},
                                   p::SystemParams;
                                   v_wind::Float64 = 11.0,
                                   omega::Float64 = 9.5,
                                   elev_deg::Float64 = 20.0)
    zs, radii, n_rings = ring_spacing_v4(design.r_hub, design.r_bottom,
    ...
    thrust   = 0.5 * rho * v_wind^2 * π * r_rotor^2 *
               ct_at_tsr(lambda_t) * cosd(elev_deg)^2.0
...
"""
function objective_v6(x::AbstractVector, beam_profile::BeamProfile,
                       p::SystemParams;
    ...
    result = design_from_vector_v6(x, beam_profile, p;
                                    max_ground_radius=max_ground_radius)
    design = result.design
    stack  = result.stack

    # Compute effective radii from expansion rotors (steady-state estimate)
    r_eff = estimate_effective_radii(design, stack, p;
                                      v_wind=v_rated, elev_deg=rad2deg(elev_angle))
    ...
    eval_result = evaluate_design(design;
                                   r_rotor=r_rotor, elev_angle=elev_angle,
                                   v_peak=v_peak, fos_req=fos_req,
                                   omega_rotor=omega, v_rated=v_rated,
                                   P_rated=power_W,
                                   max_ground_radius=max_ground_radius)
