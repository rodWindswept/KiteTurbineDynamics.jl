# scratch/r7_phys_damp.jl — with the ARTIFICIAL line damper OFF, does the load
# oscillation decay (physical damping sufficient) or persist (missing damping)?
...
p = params_5kw_188()
@printf("physical rope damping zeta = %.4f\n", p.zeta)
x = seed_genome(5.0)
...
function run(ld::Float64, secs::Float64=120.0, sample::Float64=4.0)
    u = copy(u_settled)
    Nc = Float64[]
    for _ in 1:round(Int, secs / sample)
        run_canonical_sim!(u, sys, pc, wf, round(Int, sample / dt), dt; lift_device=lift, lin_damp=ld)
...
for ld in (0.0, 0.05)
    ...
    n = length(Nc)
    h = n ÷ 2
    @printf("lin_damp=%.2f  first-half range=%.0f  second-half range=%.0f  overall min=%.0f max=%.0f\n",
        ld, maximum(Nc[1:h]) - minimum(Nc[1:h]),
        maximum(Nc[h+1:end]) - minimum(Nc[h+1:end]), minimum(Nc), maximum(Nc))
