# scratch/probe_kite_lag_damping.jl
#
# Tests the physical kite model:
#   1. Kite position P_kite in world space lags P_target = P_sa + L_line * lift_dir
#      with physical flight adjustment timescale tau_kite (e.g. 5.0 s).
#   2. High frequency (> 0.5 Hz): Kite holds station in the air mass (attenuated by omega*tau >> 1).
#   3. Low frequency (< 0.05 Hz): Kite naturally drifts downwind to track the bow,
#      keeping line tension at T_ref (~254 N) without artificial tension escalation.
#   4. Dyneema tether has physical elasticity EA = 200 kN and line damping c = 200 N*s/m.
...
function run_sim_kite_lag!(
    ...
    for step in 1:n_steps
        ...
        # ── Elastic-Damped Lift Line Force ──
        ...
        # Tension with Dyneema elasticity and line damping
        ...
        # ── Update Kite Position via Flight Lag toward Target Equilibrium ──
        # Target position is the aerodynamic equilibrium downwind of the sky anchor:
        p_target = sa_pos .+ L_line_nom .* lift_dir
        # Velocity toward target (first-order flight lag)
        v_kite .= (p_target .- p_kite) ./ tau_kite
        p_kite .+= dt .* v_kite
...
function main()
    println("="^80)
    println("PROBE: Physical Dynamic Kite with Flight Lag (tau=5.0s) & Elastic Damped Line")
    println("="^80)
    ...
    c_lift = 200.0   # N*s/m
    tau_kite = 5.0   # seconds

    println(@sprintf("Sky Anchor settled: [%.3f, %.3f, %.3f]", sa_settled[1], sa_settled[2], sa_settled[3]))
    println(@sprintf("Kite pos init:      [%.3f, %.3f, %.3f]", p_kite[1], p_kite[2], p_kite[3]))
    ...
    for i in 1:n_chunks
        ...
        run_sim_kite_lag!(
            ...
            p_kite, v_kite, lift.line_length, L0_lift, EA_lift, c_lift, tau_kite, lift_dir;
            lin_damp=0.05
        )
