# Full parameter sweep diagnostic script
using KiteTurbineDynamics, LinearAlgebra, Printf
include(joinpath(@__DIR__, "..", "test", "test_settle_preload_consistency.jl"))

function solve_axial_profile_kernel_standalone(sys, p, lift_device;
                                               wind_fn=nothing, max_iter=40,
                                               tol_N=1e-2, relax=0.55)
    lift_device === nothing && return Float64[]
    n_ring = sys.n_ring
    n_seg = n_ring - 1
    T_cyan = KiteTurbineDynamics.design_preload_from_sky_anchor(p, lift_device)
    ...
    F_top_guess = max(F_aero_z / sin(p.elevation_angle) + T_cyan, 20.0)
    ...
    # Settle equilibrium baseline to place bearing & sky anchor correctly
    _, u0_build = KiteTurbineDynamics.build_kite_turbine_system(p)
    u = KiteTurbineDynamics.settle_to_equilibrium(sys, u0_build, p; lift_device=lift_device, wind_fn=wf)
    ...
    error("Preload solver did not converge in $max_iter iterations (last residual = $(m_hub * dot(du[(3N + 3hub_gid - 2):(3N + 3hub_gid)], sd)))")
end

function run_all()
    println("Running all sweeps (beta 30, 45, 60, 70 deg and multiple ring counts)...")
    for beta_deg in (30.0, 45.0, 60.0, 70.0)
        for rc in (nothing, 4)
            ...
            F_ax, iters, residual, guess = solve_axial_profile_kernel_standalone(sys, pc, lift; wind_fn=wf)
            @printf("beta=%4.1f deg | n_lines=%d | rings=%d | iters=%2d | residual=%+.4e N | old_top=%8.2f N | new_top=%8.2f N | diff=%+7.2f N\n",
                    beta_deg, pc.n_lines, sys.n_ring, iters, residual, guess, F_ax[end], F_ax[end] - guess)
        end
    end
end
run_all()
