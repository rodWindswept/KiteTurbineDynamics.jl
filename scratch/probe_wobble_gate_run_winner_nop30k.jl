# scratch/probe_wobble_gate_run_winner_nop30k.jl
#
# VARIANT of probe_wobble_gate_run_winner.jl (2026-09-21): same full protocol,
# but the settle runs n_op=30_000 (the winner's canonical gate depth) instead
# of 300_000.  Question: does the deep settle explain the violent window?
#
# Wobble-gate FULL-PROTOCOL run, v13 WINNER variant (2026-09-21 spot-check).
# Instrument-generality check (Rod's wobble-gate scope decision, advisory item):
# the SAME protocol on a DIFFERENT machine, the last v13 winner (single rotor,
# n_lines=3, 6 rings, r_hub 4.32 m), not the 5 kW campaign seed.  The build
# mirrors scripts/ode_gate_v13.jl `gate_design`, the canonical build the
# winner's 2026-09-02 re-gate used.  The settle keeps n_op=300_000 from this
# protocol.
#
...
# ── v13 winner build (2026-09-21 spot-check) ──────────────────────────────
# Mirrors scripts/ode_gate_v13.jl `gate_design` (lines 86-136), the canonical
# build the winner passed its 2026-09-02 re-gate on.  Winner reference numbers
# (regate_verdict.md): fitness 18.49 kg, evaluator P 5.41 kW, FoS_min 17.19.
# Different ring count / taper from the campaign seed is the point of the check.
...
function main()
    ...
    @assert Nr >= 5   # winner machine has 6 rings (seed mesh has 10+); floor kept as sanity
    @assert dt_use > 0
    flush(stdout)

    t_s = @elapsed u = settle_to_operational_state(
        sys, copy(u0), p, 60.0; lift_device=lift, wind_fn=wf, n_op=30_000
    )
