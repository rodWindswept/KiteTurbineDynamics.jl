# scratch/r7_windup_long.jl — workstream step 3: does the twist saturate?
# Runs the seed for 300 s (six times the honest window) at k=2.24.
...
println(" t(s) | FoS_min | P(kW) | w_gnd | dα(deg) | twist_r")
for c in 1:15
    run_canonical_sim!(u, sys, pc, wf, round(Int, 20.0 / dt), dt; lift_device=lift, lin_damp=0.05)
    ef = KiteTurbineDynamics.capture_extended(u, sys, pc, c * 20.0, wf, lift)
    tr = KiteTurbineDynamics.twist_collapse_check(u, sys)
    @printf(" %4d  | %7.2f | %5.2f | %5.2f | %7.1f | %7.3f\n",
        c * 20, minimum(ef.ring_fos), ef.base.P_kw,
        u[6 * sys.n_total + sys.n_ring + 1], sum(ef.segment_twist_deg), tr.max_ratio)
    flush(stdout)
end
