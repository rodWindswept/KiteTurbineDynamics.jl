...
function apply_config!(name)
    b = KTD.RingAttachmentPhysics()
    if name == "default"
        # current physics
    elseif name == "notilt"
        b = KTD.RingAttachmentPhysics(tilt_enabled=false)
    elseif name == "bridletilt"
        b = KTD.RingAttachmentPhysics(bridles_use_tilt=true)
    elseif name == "notorque"
        b = KTD.RingAttachmentPhysics(bridle_axial_torque=false)
    elseif name == "notilt_notorque"
        b = KTD.RingAttachmentPhysics(tilt_enabled=false, bridle_axial_torque=false)
    elseif name == "bridletilt_notorque"
        b = KTD.RingAttachmentPhysics(
            bridles_use_tilt=true, bridle_axial_torque=false
        )
    elseif name == "all"
        b = KTD.RingAttachmentPhysics(
            tilt_enabled=false, bridles_use_tilt=true, bridle_axial_torque=false
        )
    else
...
sys, u0, p = KTD.build_system_from_v10(dec, 1.0, K_MPPT_5KW_HONEST;
...
function sample(u)
    ...
    if v_hub > 0.1
        ...
        T_thrust = 0.5 * p.rho * v_hub^2 * KTD.main_rotor_swept_area(sys) *
                   KTD.ct_at_tsr(λ) * cos(atan(h[3], hypot(h[1], h[2])))^2
    end
    ...
    attach = function (ss, se)
        if se.is_ring
            ...
            isb = ss.end_a.node_id == bear_gid
            p1, p2 =
                (isb && !KTD.RING_ATTACHMENT[].bridles_use_tilt) ?
                (perp1_shaft, perp2_shaft) : (pp1t, pp2t)
    ...
    return (; z_gap, hub_lat, bear_perp, tilt_deg, T_thrust, F_bridle, F_trpt,
        F_grav, F_pred, F_meas, cone_total=sum(cone), cone_max=maximum(cone),
        topbay=sum(cone_ring), ring_per_line=copy(cone_ring))
end
...
@printf(
    "toggles: tilt_enabled=%s bridles_use_tilt=%s bridle_axial_torque=%s\n",
    b.tilt_enabled, b.bridles_use_tilt, b.bridle_axial_torque
)
@printf(
    "L_off_nom = r_hub/tan(31deg) = %.4f m   r_hub = %.3f   m_hub = %.4f kg\n",
    L_off_nom, R_hub, (sys.nodes[hub_gid]::RingNode).mass
)
...
@printf(
    ...
    "t", "z_gap", "gap-Loff", "hub_lat", "tilt", "F_thrust", "F_bridle", "F_trpt",
    "F_grav", "F_pred", "F_meas"
)
@printf(
    "%7.3f %9.4f %+9.4f %9.4f %8.3f %9.1f %9.1f %9.1f %9.1f %9.1f %9.1f\n",
    rows[end].t, s.z_gap, s.z_gap - L_off_nom, s.hub_lat, s.tilt_deg, s.T_thrust,
    s.F_bridle, s.F_trpt, s.F_grav, s.F_pred, s.F_meas
)
...
function record!(rows, u, t, L_off_nom)
    ...
    if length(rows) <= 11 || length(rows) % 25 == 0
        @printf(
            "%7.3f %9.4f %+9.4f %9.4f %8.3f %9.1f %9.1f %9.1f %9.1f %9.1f %9.1f\n",
            t, sm.z_gap, sm.z_gap - L_off_nom, sm.hub_lat, sm.tilt_deg, sm.T_thrust,
            sm.F_bridle, sm.F_trpt, sm.F_grav, sm.F_pred, sm.F_meas
        )
    end
    return nothing
end
...
run_canonical_sim!(u, sys, p, wf, n_steps, DT;
    lift_device=lift, lin_damp=0.0, callback=cb, breaks_enabled=false)
...
@printf("\n── summary ──\n")
@printf("z_gap  t=0 %.4f  min %.4f (t=%.2f)  max %.4f (t=%.2f)  final %.4f\n",
    z[1], minimum(z), tt[argmin(z)], maximum(z), tt[argmax(z)], z[end])
@printf("gap deficit vs L_off_nom: min %+.4f  max %+.4f\n",
    minimum(z) - L_off_nom, maximum(z) - L_off_nom)
@printf("hub_lat t=0 %.4f  max %.4f (t=%.2f)  final %.4f\n",
    hl[1], maximum(hl), tt[argmax(hl)], hl[end])
@printf("tilt_deg max %.3f   cone_total min %.2f  final %.2f\n", maximum(tilt),
    minimum(ct), ct[end])
if first_dead === nothing
    @printf("cone never drops below 1.0 N\n")
else
    @printf("cone first < 1.0 N at t=%.2f s (then dead %.0f%% of samples)\n",
        tt[first_dead], 100 * count(<(1.0), ct[(first_dead + 1):end]) / max(length(ct) - first_dead, 1))
end
...
csv = joinpath(@__DIR__, "..", ".julia_depot", "logs",
    "axialgap_$(ISLAND)_$(CONFIG).csv")
open(csv, "w") do io
    println(io, "t,z_gap,hub_lat,bear_perp,tilt_deg,F_thrust,F_bridle,F_trpt,F_grav,F_pred,F_meas,cone_total,topbay")
    for r in rows
        @printf(io, "%.4f,%.6f,%.6f,%.6f,%.4f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
            r.t, r.z_gap, r.hub_lat, r.bear_perp, r.tilt_deg, r.T_thrust, r.F_bridle,
            r.F_trpt, r.F_grav, r.F_pred, r.F_meas, r.cone_total, r.topbay)
    end
end
println("csv: $csv")
println("=== done ===")
