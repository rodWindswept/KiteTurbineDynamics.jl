# scratch/r7_balance.jl — at the RUNNING state, what rope torque does ring 1
# actually feel vs the generator torque it should balance?
...
function balance(tag)
    ...
    fill!(torques, 0.0)
    KiteTurbineDynamics.compute_rope_forces!(forces, torques, u, @view(u[(6N+1):(6N+Nr)]),
        sys, p, wf, 0.0, pp1, pp2)
    ω = u[6N + Nr + 1]
    τ_gen = sys.k_mppt_ref[] * ω^2
    δ1 = rad2deg(u[6N + 2] - u[6N + 1])
    # net α-acceleration the ODE would apply on ring 1
    @printf("%-8s δ1=%6.2f°  rope τ(ring1)=%12.1f  τ_gen=%8.1f  ratio=%8.1f\n",
        tag, δ1, torques[1], τ_gen, torques[1] / max(τ_gen, 1e-9))
end
balance("settle")
for k in 1:4
