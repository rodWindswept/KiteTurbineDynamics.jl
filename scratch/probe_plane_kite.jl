# scratch/probe_plane_kite.jl
#
# Physics of the Lifter Kite:
#   1. Aerodynamic Directional Stability (Y-axis):
#      The wind is parallel to +X. A trimmed foil/sled kite is aerodynamically stable
#      about the vertical symmetry plane Y = 0. Its crosswind position is anchored to Y = 0
#      with aerodynamic side-force damping.
#   2. Catenary Tracking (X-Z plane):
#      In the X-Z plane, the kite tracks the slow catenary bowing of the sky anchor
#      (P_target = [sa_pos[1] + L*cos(theta), 0.0, sa_pos[3] + L*sin(theta)])
#      so line tension stays strictly at T_ref (~254 N) without artificial tension escalation.
#   3. Tether Elasticity & Damping:
#      Dyneema tether has EA = 200 kN and acoustic/viscoelastic damping c = 200 N*s/m.
...
function run_sim_plane_kite!(
    ...
    for step in 1:n_steps
        ...
        # In Y: kite is pinned at Y = 0 by aerodynamic symmetry (crosswind directional stability)
        # In X-Z: kite tracks slow catenary drift of sky anchor via low-pass filter (tau_xz)
        sa_pos = @view u[(3 * (sa_gid - 1) + 1):(3 * sa_gid)]
        sa_vel = @view u[(3 * N + 3 * (sa_gid - 1) + 1):(3 * N + 3 * sa_gid)]

        v_filt_x = (sa_pos[1] - p_sa_filt_xz[1]) / tau_xz
        v_filt_z = (sa_pos[3] - p_sa_filt_xz[2]) / tau_xz
        p_sa_filt_xz[1] += dt * v_filt_x
        p_sa_filt_xz[2] += dt * v_filt_z

        p_kite[1] = p_sa_filt_xz[1] + L_line_nom * cos_el
        p_kite[2] = 0.0   # aerodynamically centered in the wind symmetry plane
        p_kite[3] = p_sa_filt_xz[2] + L_line_nom * sin_el
...
function main()
    println("="^80)
    println("PROBE: Physical Lifter (Y=0 Aero Symmetry + Slow X-Z Catenary Tracking)")
    println("="^80)
    ...
    p_sa_filt_xz = [sa_settled[1], sa_settled[3]]

    p_kite = [sa_settled[1] + lift.line_length * cos(θ_lift), 0.0, sa_settled[3] + lift.line_length * sin(θ_lift)]
    v_kite = zeros(3)
    ...
    c_lift = 200.0   # N*s/m
    tau_xz = 15.0    # slow catenary tracking timescale

    println(@sprintf("Sky Anchor settled: [%.3f, %.3f, %.3f]", sa_settled[1], sa_settled[2], sa_settled[3]))
    println(@sprintf("T_ref: %.1f N, L_line: %.2f m, L0: %.4f m, c_lift: %.1f N*s/m, tau_xz: %.1f s",
        T_ref, lift.line_length, L0_lift, c_lift, tau_xz))
    ...
    for i in 1:n_chunks
        ...
        run_sim_plane_kite!(
            ...
            p_kite, v_kite, p_sa_filt_xz, lift.line_length, L0_lift, EA_lift, c_lift, tau_xz, θ_lift;
            lin_damp=0.05
        )
