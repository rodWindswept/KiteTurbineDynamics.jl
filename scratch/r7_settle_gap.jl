# scratch/r7_settle_gap.jl — per-segment twist: settle vs running ODE, and the
# settle chain's torque target vs the ODE's measured segment torque.
...
function report(tag)
    ...
    ds = ef.segment_twist_deg
    ts = ef.segment_torque
    @printf("%-8s cum=%6.1f°  per-seg twist= %s\n", tag, sum(ds),
        join(round.(ds, digits=1), ","))
    @printf("         seg torque = %s\n", join(round.(ts, digits=0), ","))
end
@printf("omega_eq pinned by settle; tau_gen = k*w^2\n")
report("settle")
for k in 1:4
