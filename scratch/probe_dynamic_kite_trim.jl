# scratch/probe_dynamic_kite_trim.jl
#
# Tests a 3-DOF Dynamic Lifter Kite with:
#   1. Inertial mass m_kite = 5.0 kg in 3D Cartesian space.
#   2. Aerodynamic trim restoring stiffness k_trim = T_ref / L_line (~10.2 N/m)
#      which models the kite's bridle trim restoring it toward r_eq = L0 * u_trim.
#   3. Aerodynamic velocity damping c_aero (~23-46 N*s/m).
#   4. Elastic tether (EA = 200 kN, c_line = 200 N*s/m).
...
function run_sim_dynamic_kite!(
    ...
    for step in 1:n_steps
        ...
        # Aerodynamic force on kite:
        # 1. Trim equilibrium force
        # 2. Aerodynamic restoring force toward bridle trim direction relative to sky anchor
        r_dev = r_line .- r_eq_trim
        F_aero_restore = -k_trim .* r_dev

        # 3. Aerodynamic velocity damping
        ...
        F_aero_total = F_aero_trim .+ F_aero_restore .+ F_aero_damp
...
function main()
    println("="^80)
    println("PROBE: 3-DOF Dynamic Lifter Kite with Aerodynamic Trim Restoring")
    println("="^80)
    ...
    r_eq_trim = lift.line_length .* lift_dir
    p_kite = sa_settled .+ r_eq_trim
    v_kite = zeros(3)
    ...
    k_trim = T_ref / lift.line_length         # ~10.2 N/m
    ...
    println(@sprintf("k_trim: %.2f N/m, c_par: %.1f N*s/m, c_perp: %.1f N*s/m",
        k_trim, c_aero_par, c_aero_perp))
    ...
    for i in 1:n_chunks
        ...
        run_sim_dynamic_kite!(
            ...
            F_aero_trim, r_eq_trim, k_trim, c_aero_par, c_aero_perp;
            lin_damp=0.05
        )
