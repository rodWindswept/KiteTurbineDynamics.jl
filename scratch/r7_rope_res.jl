# scratch/r7_rope_res.jl — effect of rope sub-segment count on the shaft wobble
# and the transmission-ring load. Run once per ROPE_SUBSEGS value.
...
ring_lat(k) = (gid = sys.ring_ids[k]; c = u[(3*(gid-1)+1):(3*gid)];
               norm(c .- dot(c, shaft) .* shaft))

Nc = Float64[]; r9 = Float64[]; fmin = Float64[]
for _ in 1:10
    run_canonical_sim!(u, sys, pc, wf, round(Int, 1.0 / dt), dt; lift_device=lift, lin_damp=0.05)
    ef = KiteTurbineDynamics.capture_extended(u, sys, pc, 0.0, wf, lift)
    push!(Nc, ef.ring_Ncomp[1]); push!(r9, ring_lat(9)); push!(fmin, minimum(ef.ring_fos))
end
@printf("ROPE_SUBSEGS=%d  n_total=%d  dt=%.3e\n", ROPE_SUBSEGS, N, dt)
@printf("  N_ring2  %.0f .. %.0f (range %.0f)   ring9 lateral %.3f .. %.3f m   FoS_min %.2f\n",
    minimum(Nc), maximum(Nc), maximum(Nc) - minimum(Nc), minimum(r9), maximum(r9), minimum(fmin))
