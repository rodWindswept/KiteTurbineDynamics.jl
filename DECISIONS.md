...
**Method.** New probe `scratch/probe_axial_gap_compare.jl`, run on islands 1, 2 and 3 with the canonical integrator. It logs the axial gap `z_gap = (p_bear − p_hub)·ŝ` against the nominal `r_hub/tan(31°)`, the ring tilt, the hub's axial force budget (`F_thrust + F_bridle − F_trpt − F_grav`, cross-checked against `m_hub·(du_hub·ŝ)`), and the cone and top-bay tensions. Three experiment toggles were added to `src/rope_forces.jl` as `RING_ATTACHMENT`, all defaulting to the shipped physics so nothing changes without a ruling.

**Findings.**

1. **The axial compression does NOT lead the lateral drift.** At t = 0.49 s island 1's hub has already drifted **0.874 m** while `z_gap` is unchanged to 0.5 mm. The gap only moves later, reaching **−0.169 m** at t ≈ 0.89 s, and the cone dies at t = 1.36 s. So the ordering is lateral drift, then compression, then cone death. The compression is a consequence, not the initiator. It is also the cleanest discriminator measured so far: island 2 reaches −0.0011 m and island 3 −0.0024 m, so island 1's excursion is about **70×** larger. Because the bridle's slack tolerance is only ~0.7 mm, that 0.169 m is roughly 240× more than enough to unload the cone.
2. **Island 1's top bay is not under-tensioned. It is oscillating.** Across all runs the top-bay mean is 1.77 to 1.89 kN. The difference is variance: island 1 swings **276 %** (356 → 5588 N) over 2 s, while islands 2 and 3 swing **16 %** and **19 %** (1505 → 1861 N). So the earlier "top bay drops to 90 N" reading was a trough of a violent oscillation on an otherwise equally-tensioned bay. Something is pumping spurious axial strain into island 1's top bay.
3. **The dual-basis attachment conflict is real, and it is the dominant driver of the cone decoupling.** `rope_forces.jl:269` resolves a ring attachment as `is_bridle ? (perp1_shaft, perp2_shaft) : (pp1_tilt, pp2_tilt)`, and `is_bridle` tests only `end_a`. Bridles are stored bearing-first (`initialization.jl:347`), so every bridle's HUB end takes the fixed shaft plane while the hub's TRPT lines take the synthetic tilted plane. A rigid ring has ONE plane; the model gives it two circles out of plane by up to the 30° clamp. **Aligning the bridles with the ring plane transforms the result.** Island 1's cone mean load rises from **3.87 N to 1382 N** and its minimum from **0 to 74 N**; the gap compression halves (−0.169 → −0.089 m). Island 3 becomes **perfectly steady** over the full 120 s: hub and bearing p2p **0.0000 m**, cyan p2p 0.02 N, and a steady **158.9 N** cone with **zero** slack (its minimum was 0 N before).
4. **The off-axis bridle wrap torque is a second, independent driver.** `rope_forces.jl:359` projects the rope moment onto the shaft axis. For a bridle the attachment is out of the ring plane, so that moment is spurious. Clamping it for bridles alone keeps island 1's cone alive, minimum **0 → 37.8 N**. It is **not additive** with finding 3: the plane fix alone gives 86.8 N and plane-plus-no-torque gives 87.3 N. So the plane fix dominates and the torque term becomes benign once the bridle lies in the ring plane.
5. **The tilt itself is NOT the artifact.** Removing it entirely (`tilt_enabled = false`) makes island 1 WORSE: gap compression grows from 0.169 to 0.394 m and the cone dies sooner (t = 0.96 s). So the tilted plane is the better approximation of the ring's true orientation, and the bridle's forced return to the shaft plane is the defect. This also reframes the source comment "Bridle geometry stays in the shaft frame to prevent feedback instability" as a hack that masked, and partly caused, the instability.

**Decided.** The three toggles are added and **all default to the shipped physics**, so no behaviour changed and the unit suite stays green at 2163/2163. **`bridles_use_tilt = true` is proposed as the fix and awaits a ruling**, because it changes every machine's bridle geometry and moves island 1's FoS.

**Consequences.**
- **The fix is not sufficient for island 1.** With `bridletilt` its 120 s gate reads FoS trough **0.7400** against **0.9079** at the shipped setting, and hub p2p stays at 1.54 m. So the lateral runaway persists on island 1 alone. Island 1 is the only 3-rotor machine in the set, so the remaining mechanism is probably expansion-rotor-specific.
- The FoS trough moving from 0.9079 to 0.7400 is not simply a regression. At the shipped setting island 1's cone is dead (mean 3.87 N), so the lift chain is disconnected and the recorded loads are chaotic. With the planes joined the cone carries a mean **1382 N**, so the machine is genuinely loaded and its low FoS is real. Island 1 (33.68 kg, the heaviest candidate) looks structurally marginal once the load path is connected.
- **Two latent defects recorded.** The tilt is a synthetic one-way coupling with an unanchored constant (`TILT_SCALE = 0.1` rad per metre of bearing drift, clamped at 30°), which belongs in the trust log's Unanchored Parameters. And `_tilted_ring_basis` is documented "for dashboard visualization" while driving physics.

**Status:** Active. Diagnosis complete and cross-logged. The plane fix awaits a ruling; island 1's remaining mechanism is open.
...
3. **`F_radial` is a rim load, never a centre-of-mass force.** The block is removed and the rule recorded in source. The designed radial restraint (the spoke spring, `ring_forces.jl:376-396`) stays **INERT BY RULING (Rod, 2026-09-22)**, not by oversight. Only the ground ring and the back-line anchor touch the ground, so a TRPT column is a free-floating tensegrity with no centreline anchor. Energising the spokes would add an artificial guide-wire pulling every ring toward the ground station's line of sight. Rings hold radial integrity through line tension and inter-bay truss geometry. Do not switch the spokes on as a stability crutch.
...
5. **The back-line ground-anchor offset is a PLACEMENT constant, set to 11.0 m (RULED, Rod, 2026-09-22).** `BACK_ANCHOR_FWD_X_M = 11.0` in `src/parameters.jl`, and `mass_scale` no longer multiplies it by `geom_scale`. Measured levers for the island 1 choke (design-time split, `scratch/probe_backanchor_sweep.jl`): at the old length-scaled `back_anchor_fwd_x = 6.901 m` the demanded `T_back` was **365.3 N**, above the 320 N stop, so the anchor was forced onto the Dyneema. At `fwd_x = 11.0 m` the demand falls to **315.8 N** (soft-capable) and `T_cyan` rises 149.8 → **270.96 N**. The geometric reading matches the field note: against the axial projection the ground anchor was **1.19 m UPWIND** at 6.901 m and is **2.91 m DOWNWIND** at 11.0 m, which is the field setup. `params_daisy`'s old `3.78 = 11.0 x (10.31/30)` length-ratio law is superseded; the 1.5 kW daisy anchor therefore moves too.
...
- **The fixes are real, and they do NOT rescue island 1.** Both 120 s gates re-ran at `lin_damp = 0.00` on the remediated build. Island 3 (single rotor) is clean: FoS trough **12.3348** against 12.2739 before, hub p2p 0.0074 m, bridle dips still 0.100 s contiguous and well inside the 0.8 s bar. Island 1 is still FAIL, and worse on the cone: FoS trough **1.1269**, hub p2p **2.1037 m**, bearing p2p 2.8898 m, cone mean tension **0.0000 N** for the whole window, and contiguous cone slack **22.62 s** against 14.93 s before. So the four defects are genuine, and none of them causes the collapse. Fix 3 moves the operating point, because the preload profile feeds `trpt_matched_place`, which sets the placed geometry and the bridle cut length. On island 1 that move is adverse. The proximate mechanism is untouched: the bow compresses the bearing-to-hub gap and the bearing follows the bow.
- **The back-line offset ruling landed and its mechanical goals are met, but it does NOT rescue island 1 either.** With `BACK_ANCHOR_FWD_X_M = 11.0` both gates re-ran at `lin_damp = 0.00`.
  - **Island 3 improves on EVERY metric.** FoS trough **13.4653** (12.3348 before, 12.2739 pre-remediation). Hub p2p **0.0047 m** (0.0075). Cone mean tension **104.69 N** (47.6, and 45.7 before that) — more than double. Contiguous bridle slack **0.08 s** (0.10), inside the 0.8 s bar. Total bridle slack 25.8 s (58.9). Cyan mean 146.3 N (96.8). P_gen 5.687 kW (5.6296). The un-choked load path is a clear win on the machine that was already stable.
  - **Island 1 still FAILS and its FoS trough is worse.** FoS trough **0.9079** (1.1269 at 6.901, 1.5865 pre-remediation). Hub p2p 1.7092 m (2.1037). Contiguous cone slack 14.9 / 14.5 / 12.6 s, against the 0.8 s bar. The choke IS released, and it helped specific symptoms: cone mean tension rises from **0.0000 N to 3.8700 N**, the cyan peak falls from 2034.9 N to 1197.8 N, and the back line now swings inside its elastic travel (70 to 290 N over the relax, mean 189.2 N) instead of sitting pinned on the stop. So the choke was a real defect worth fixing, and it was NOT island 1's binding constraint.
  - **Conclusion.** Island 1's collapse has a different dominant cause. The proximate mechanism stands: the bow compresses the bearing-to-hub gap and the bearing follows the bow. Two levers are now exhausted (the thrust/preload/mass corrections and the back-line offset), and neither clears the gate.
- **The L/r 2.0 realisability fixture is now geometry-pinned.** The offset ruling moved its `F_ax[end]` from 1163.72 N to 1278.63 N and shifted all nine pinned section A values, because the fixture inherited the global `back_anchor_fwd_x`. Rather than a fifth re-baseline, section A now pins `back_anchor_fwd_x = 6.901`, the value it was MEASURED at. That is the same test-coupling defect the file's own 2026-09-16 note records. The shipped default is still covered end-to-end by sections B and C. No re-baseline was needed: the file returns 32/32.
- **Acceptance suite: 7 of 8 pass.** `test/acceptance_runtests.jl` returned PASS for `test_evaluator_v13`, `test_gate_v13`, `test_rope_break`, `test_rotor_power_realism`, `test_settle_drag_alignment`, `test_physics_path_ode` and `test_jtheta_no_reversal`. Only `test_settle_lowk_honest` failed, on its A2 assertion.
- **`test_settle_lowk_honest.jl` A2 is re-baselined, with attribution.** The point sits at k = 2.0 against a 5 kW floor. The campaign's honest k (2.24) used to sustain 5.12 kW, so k = 2.0 undershot and the run rejected. The fixes above raise the sustained power, so k = 2.0 now clears the floor and returns `:ok`. Attribution was measured, not assumed: with `src/` at `d130632` the file is **13/13**, and with the fixes it is **12/13** on exactly that assertion (`ok === reject`). The verdict is no longer pinned, because pinning a marginal floor verdict is what let the point go stale. The file's real guard still holds: a run in the floor region must carry its measured window statistics and must not disguise either outcome as 0 kW and FoS = Inf. The rejected-evaluation path stays covered by A1 at k = 0.5. Re-verified **13/13** after the change.

**Status:** Active. Fixes 1 to 4 landed (`57b3f58`), the offset ruling landed, and the spoke restraint is ruled inert. Full suite 2163/2163 green at `fwd_x = 11.0`. Island 1 remains the open problem, and its dominant mechanism is still unidentified.
